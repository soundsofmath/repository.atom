...
from shazamscrapers.modules import cleandate
...
V2_API_KEY = control.addon('plugin.video.shazam').getSetting('trakt.client_id')
CLIENT_SECRET = control.addon('plugin.video.shazam').getSetting('trakt.client_secret')
REDIRECT_URI = 'urn:ietf:wg:oauth:2.0:oob'

if V2_API_KEY == "" or CLIENT_SECRET == "":
    V2_API_KEY = base64.b64decode("ff13321831ab46a34108ce8b8d9adbb106b5ed460f88303eeb0b2d9534c6dc83")
    CLIENT_SECRET = base64.b64decode("2c5cc70a7bb8a030e09193803e773579cf529855210f47ff1f4c53feeb4f206c")
...
def __getTrakt(url, post=None):
    try:
        ...
        if getTraktCredentialsInfo():
            headers.update({'Authorization': 'Bearer %s' % control.addon('plugin.video.shazam').getSetting('trakt.token')})
        ...
        opost = {'client_id': V2_API_KEY, 'client_secret': CLIENT_SECRET, 'redirect_uri': REDIRECT_URI, 'grant_type': 'refresh_token', 'refresh_token': control.addon('plugin.video.shazam').getSetting('trakt.refresh')}
    ...
    try:
        if getTraktCredentialsInfo() == True:
            if control.yesnoDialog(control.lang(32511) + '[CR]' + control.lang(32512), heading='Trakt'):
                control.addon('plugin.video.shazam').setSetting(id='trakt.user', value='')
                control.addon('plugin.video.shazam').setSetting(id='trakt.authed', value='')
                control.addon('plugin.video.shazam').setSetting(id='trakt.token', value='')
                control.addon('plugin.video.shazam').setSetting(id='trakt.refresh', value='')
            raise Exception()
        ...
        print('info - ' + token)
        control.addon('plugin.video.shazam').setSetting(id='trakt.user', value=user)
        control.addon('plugin.video.shazam').setSetting(id='trakt.authed', value=authed)
        control.addon('plugin.video.shazam').setSetting(id='trakt.token', value=token)
        control.addon('plugin.video.shazam').setSetting(id='trakt.refresh', value=refresh)
        raise Exception()
    except:
...
def getTraktCredentialsInfo():
    user = control.addon('plugin.video.shazam').getSetting('trakt.user').strip()
    token = control.addon('plugin.video.shazam').getSetting('trakt.token')
    refresh = control.addon('plugin.video.shazam').getSetting('trakt.refresh')
    if (user == '' or token == '' or refresh == ''): return False
    return True


def getTraktIndicatorsInfo():
    indicators = control.addon('plugin.video.shazam').getSetting('indicators') if getTraktCredentialsInfo() == False else control.addon('plugin.video.shazam').getSetting('indicators.alt')
...
def cachesyncMovies(timeout=0):
    indicators = cache.get(syncMovies, timeout, control.addon('plugin.video.shazam').getSetting('trakt.user').strip())
    return indicators


def timeoutsyncMovies():
    timeout = cache.timeout(syncMovies, control.addon('plugin.video.shazam').getSetting('trakt.user').strip())
    return timeout
...
def cachesyncTVShows(timeout=0):
    indicators = cache.get(syncTVShows, timeout, control.addon('plugin.video.shazam').getSetting('trakt.user').strip())
    return indicators


def timeoutsyncTVShows():
    timeout = cache.timeout(syncTVShows, control.addon('plugin.video.shazam').getSetting('trakt.user').strip())
