...
INCLUDESHOWBOXARIZE = ADDON.getSetting('includeshowboxarize')
INCLUDEGURZIL  = ADDON.getSetting('includegurzil')
...
def getCacheSize():
	...
	dbfiles   = [
		...
		(os.path.join(ADDONDATA, 'plugin.video.lastship', 'cache.db')),
		(os.path.join(ADDONDATA, 'plugin.video.covenant', 'cache.db')),
		(os.path.join(ADDONDATA, 'plugin.video.exodus', 'cache.db')),
		(os.path.join(ADDONDATA, 'plugin.video.flixnet', 'cache.db')),
		(os.path.join(ADDONDATA, 'plugin.video.incursion', 'cache.db')),
		(os.path.join(ADDONDATA, 'plugin.video.placenta', 'cache.db')),
		(os.path.join(ADDONDATA, 'plugin.video.genesisreborn', 'cache.db')),
		(os.path.join(ADDONDATA, 'plugin.video.showboxarize', 'cache.db')),
		(os.path.join(ADDONDATA, 'plugin.video.gurzil', 'cache.db'))]
	cachelist = [
		(PROFILEADDONDATA),
		(ADDONDATA),
	...
	if INCLUDEVIDEO == 'true':
		...
		if INCLUDEALL == 'true': files = dbfiles
			...
			if INCLUDELASTSHIP == 'true':     files.append(os.path.join(ADDONDATA, 'plugin.video.lastship', 'cache.db'))
			...
			if INCLUDEFLIXNET == 'true': files.append(os.path.join(ADDONDATA, 'plugin.video.flixnet', 'cache.db'))
			if INCLUDEINCURSION == 'true':  files.append(os.path.join(ADDONDATA, 'plugin.video.incursion', 'cache.db'))
			if INCLUDEPLACENTA == 'true':  files.append(os.path.join(ADDONDATA, 'plugin.video.placenta', 'cache.db'))
			if INCLUDEGENESISREBORN == 'true':  files.append(os.path.join(ADDONDATA, 'plugin.video.genesisreborn', 'cache.db'))
			if INCLUDESHOWBOXARIZE == 'true':  files.append(os.path.join(ADDONDATA, 'plugin.video.showboxarize', 'cache.db'))
			if INCLUDEGURZIL == 'true':  files.append(os.path.join(ADDONDATA, 'plugin.video.gurzil', 'cache.db'))
		if len(files) > 0:
	...
	if os.path.exists(dep):
		...
		for depends in match:
			if not 'xbmc.python' in depends:
				...
				if not DP == None: 
					DP.update("","Checke Abhangigkeiten [COLOR yellow]%s[/COLOR] von [COLOR yellow]%s[/COLOR]" % (depends, name),"")
				if os.path.exists(dependspath):
					toggleAddon(name, 'true')
			xbmc.sleep(100)

def toggleAdult():
	do = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du die Adult Addons [COLOR %s]Aktivieren[/COLOR] oder [COLOR %s]Deaktivieren[/COLOR] ?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR lime]Aktivieren[/COLOR][/B]", nolabel="[B][COLOR red]Deaktivieren[/COLOR][/B]")
	...
	if len(found) > 0: 
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du eine Liste aller Addons sehen die %s sind?[/COLOR]" % (COLOR2, goto.replace('ing', 'ed')), yeslabel="[B][COLOR lime]JA, anzeigen[/COLOR][/B]", nolabel="[B][COLOR red]NEIN, abbrechen[/COLOR][/B]"):
			editlist = '[CR]'.join(found)
			TextBox(ADDONTITLE, "[COLOR %s]Hier is eine Liste aller Addons die %s mit Adult Inhalten:[/COLOR][CR][CR][COLOR %s]%s[/COLOR]" % (COLOR1, goto.replace('ing', 'ed'), COLOR2, editlist))
		...
		forceUpdate(True)
	else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Keine Adult Addons gefunden[/COLOR]" % COLOR2)
...
def toggleAddon(id, value, over=None):
	...
	if 'error' in response and over == None:
		...
		DIALOG.ok(ADDONTITLE, "[COLOR %s]Fehler %s [COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, v, id), "Checke ob die Addon Liste aktuell ist und versuche es erneut.[/COLOR]")
		forceUpdate()
...
def createQR():
	url = getKeyboard('', "%s: Gib die URL fur den QRCode ein." % ADDONTITLE)
	if url == "": LogNotify("[COLOR %s]Erstelle QR[/COLOR]" % COLOR1, '[COLOR %s]QRCode erstellen abgebrochen![/COLOR]' % COLOR2); return
	if not url.startswith('http://') and not url.startswith('https://'): LogNotify("[COLOR %s]Erstelle QR[/COLOR]" % COLOR1, '[COLOR %s]URL nicht korrekt![/COLOR]' % COLOR2); return
	if url == 'http://' or url == 'https://': LogNotify("[COLOR %s]Erstelle QR[/COLOR]" % COLOR1, '[COLOR %s]URL nicht korrekt![/COLOR]' % COLOR2); return
	working = workingURL(url)
	if not working == True:
		if not DIALOG.yesno(ADDONTITLE, "[COLOR %s]Deine URL funnktioniert nicht, willst du trotzdem fortfahren?[/COLOR]" % COLOR2, "[COLOR %s]%s[/COLOR]" % (COLOR1, working), yeslabel="[B][COLOR red]Ja, erstellen[/COLOR][/B]", nolabel="[B][COLOR lime]NEIN, abbrechen[/COLOR][/B]"):
			return
	...
	DIALOG.ok(ADDONTITLE, "[COLOR %s]Der QRCode Wurde erstellt und befindet sich im AddonData Ordner:[/COLOR]" % COLOR2, "[COLOR %s]%s[/COLOR]" % (COLOR1, image.replace(HOME, '')))
...
def cleanupBackup():
	...
	list = []; filelist = []
	if len(folder) == 0:
		LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Backup Pfad: Leer[/COLOR]" % (COLOR2))
		return
	...
	selected = DIALOG.select("%s: Wahle Dateien zum Entfernen aus 'MyBuilds'." % ADDONTITLE, list)
	
	if selected == -1:
		LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Reinigung abgebrochen![/COLOR]" % COLOR2)
	elif selected == 0: 
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du alle Dateien im 'My_Builds' Ordner entfernen?[/COLOR]" % COLOR2, "[COLOR %s]%s[/COLOR]" % (COLOR1, MYBUILDS), yeslabel="[B][COLOR lime]JA, entfernen[/COLOR][/B]", nolabel="[B][COLOR red]NEIN, abbrechen[/COLOR][/B]"):
			clearedfiles, clearedfolders = cleanHouse(xbmc.translatePath(MYBUILDS))
			LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Entfernte Dateien: [COLOR %s]%s[/COLOR] / Ordner:[/COLOR] [COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, clearedfiles, COLOR1, clearedfolders))
		else:
			LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Reinigen abgebrochen![/COLOR]" % COLOR2)
	else:
		path = filelist[selected-1]; passed = False
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du [COLOR %s]%s[/COLOR] vom 'My_Builds' Ordner entfernen?[/COLOR]" % (COLOR2, COLOR1, list[selected]), "[COLOR %s]%s[/COLOR]" % (COLOR1, path), yeslabel="[B][COLOR lime]JA, entfernen[/COLOR][/B]", nolabel="[B][COLOR red]NEIN, abbrechen[/COLOR][/B]"):
			...
			else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Fehler beim Reinigen %s![/COLOR]" % (COLOR2, list[selected]))
		else:
			LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Reinigung abgebrochen![/COLOR]" % COLOR2)
...
def forceUpdate(silent=False):
	...
	if silent == False: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Erzwinge Addon Updates[/COLOR]' % COLOR2)

def convertSpecial(url, over=False):
	total = fileCount(url); start = 0
	DP.create(ADDONTITLE, "[COLOR %s]Konvertiere Physical Pfad zu Special" % COLOR2, "", "Bitte Warten[/COLOR]")
	for root, dirs, files in os.walk(url):
		for file in files:
			start += 1
			perc = int(percentage(start, total))
			if file.endswith(".xml") or file.endswith(".hash") or file.endswith("properies"):
				DP.update(perc, "[COLOR %s]Scanne: [COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, root.replace(HOME, '')), "[COLOR %s]%s[/COLOR]" % (COLOR1, file), "Bitte Warten[/COLOR]")
	...
	if over == False: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Pfade konvertieren fertig![/COLOR]" % COLOR2)
...
def clearCrash():  
	...
	for file in glob.glob(os.path.join(LOG, '*crashlog*.*')):
		files.append(file)
	if len(files) > 0:
		if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Willst du die Crash Logs entfernen?' % COLOR2, '[COLOR %s]%s[/COLOR] Dateien gefunden[/COLOR]' % (COLOR1, len(files)), yeslabel="[B][COLOR lime]JA, entfernen[/COLOR][/B]", nolabel="[B][COLOR red]NEIN, abbrechen[/COLOR][/B]"):
			for f in files:
				os.remove(f)
			LogNotify('[COLOR %s]Entferne Crash Logs[/COLOR]' % COLOR1, '[COLOR %s]%s Crash Logs entfernt[/COLOR]' % (COLOR2, len(files)))
		else: LogNotify('[COLOR %s]%s[/COLOR]' % (COLOR1, ADDONTITLE), '[COLOR %s]Crash Logs etnfernen abgebrochen[/COLOR]' % COLOR2)
	else: LogNotify('[COLOR %s]Entferne Crash Logs[/COLOR]' % COLOR1, '[COLOR %s]Keine Crash Logs gefunden[/COLOR]' % COLOR2)

def hidePassword():
	if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du alle Passworteingaben in den Addon Einstellungen [COLOR %s]verbergen[/COLOR] ?[/COLOR]" % (COLOR2, COLOR1), yeslabel="[B][COLOR lime]JA, verbergen[/COLOR][/B]", nolabel="[B][COLOR red]NEIN, abbrechen[/COLOR][/B]"):
		count = 0
		...
		LogNotify("[COLOR %s]Passwort verbergen[/COLOR]" % COLOR1, "[COLOR %s]%s Dateien geandert[/COLOR]" % (COLOR2, count))
		log("[Hide Passwords] %s items changed" % count, xbmc.LOGNOTICE)
	else: log("[Hide Passwords] Cancelled", xbmc.LOGNOTICE)

def unhidePassword():
	if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du alle Passworteingaben in den Addon Einstellungen [COLOR %s]anzeigen[/COLOR] ?[/COLOR]" % (COLOR2, COLOR1), yeslabel="[B][COLOR lime]JA, anzeigen[/COLOR][/B]", nolabel="[B][COLOR red]NEIN, abbrechen[/COLOR][/B]"):
		count = 0
		for folder in glob.glob(os.path.join(ADDONS, '*/')):
			sett = os.path.join(folder, 'resources', 'settings.xml')
			if os.path.exists(sett):
					...
					if 'pass' in line:
						if 'option="hidden"' in line:
							try:
								...
								pass
				f2 = open(sett, mode='w'); f2.write(f); f2.close()
		LogNotify("[COLOR %s]Passwort anzeigen[/COLOR]" % COLOR1, "[COLOR %s]%s Dateien geandert[/COLOR]" % (COLOR2, count))
...
def wizardUpdate(startup=None):
	if workingURL(WIZARDFILE):
		...
		zip = checkWizard('zip')
		if ver > VERSION:
			yes = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Es gibt eine neue Version von [COLOR %s]%s[/COLOR]!' % (COLOR2, COLOR1, ADDONTITLE), 'Willst du sie herunter laden [COLOR %s]v%s[/COLOR]?[/COLOR]' % (COLOR1, ver), nolabel='[B][COLOR red]NEIN, abbrechen[/COLOR][/B]', yeslabel="[B][COLOR lime]JA, Wizard updaten[/COLOR][/B]")
			if yes:
				log("[Auto Update Wizard] Installing wizard v%s" % ver, xbmc.LOGNOTICE)
				DP.create(ADDONTITLE,'[COLOR %s]Lade Update...' % COLOR2,'', 'Bitte Warten[/COLOR]')
				...
				ebi('UpdateLocalAddons()')
				xbmc.sleep(1000)
				LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE),'[COLOR %s]Add-on upgedated[/COLOR]' % COLOR2)
			...
			if not startup: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Keine neue Version des Wizards vorhanden[/COLOR]" % COLOR2)
...
def convertText():
	...
	DP.create(ADDONTITLE,'[COLOR %s][B]Konvertiere Text:[/B][/COLOR]' % (COLOR2),'', 'Bitte Warten')
	
	if not BUILDFILE == 'http://':
		filename = os.path.join(TEXTFILES, 'builds.txt')
		writing = '';x = 0
		a = openURL(BUILDFILE).replace('\n','').replace('\r','').replace('\t','')
		DP.update(0,'[COLOR %s][B]Konvertiere Text:[/B][/COLOR] [COLOR %s]Builds.txt[/COLOR]' % (COLOR2, COLOR1),'', 'Bitte Warten')
		...
		if len(match2) == 0:
			for name, version, url, gui, kodi, theme, icon, fanart in match:
				...
				if not theme == 'http://':
					...
					DP.update(0,'[COLOR %s][B]Konvertiere Text:[/B][/COLOR] [COLOR %s]%s_theme.txt[/COLOR]' % (COLOR2, COLOR1, name),'', 'Bitte Warten')
	...
	if not APKFILE == 'http://':
		...
		DP.update(0,'[COLOR %s][B]Konvertiere Text:[/B][/COLOR] [COLOR %s]Apks.txt[/COLOR]' % (COLOR2, COLOR1), '', 'Bitte Warten')
	...
	if not YOUTUBEFILE == 'http://':
		...
		DP.update(0,'[COLOR %s][B]Konvertiere Text:[/B][/COLOR] [COLOR %s]YouTube.txt[/COLOR]' % (COLOR2, COLOR1), '', 'Bitte Warten')
	...
	if not ADVANCEDFILE == 'http://':
		...
		DP.update(0,'[COLOR %s][B]Konvertiere Text:[/B][/COLOR] [COLOR %s]AdvancedSettings.txt[/COLOR]' % (COLOR2, COLOR1), '', 'Bitte Warten')
		match = re.compile('name="(.+?)".+?rl="(.+?)".+?con="(.+?)".+?anart="(.+?)".+?escription="(.+?)"').findall(a)
		for name, url, icon, fanart, description in match:
			x += 1
			DP.update(int(percentage(x, len(match))), '', "[COLOR %s]%s[/COLOR]" % (COLOR1, name))
	...
	DP.close()
	DIALOG.ok(ADDONTITLE, '[COLOR %s]Deine TXT Datei wurde auf 0.1.7 konvertiert und befindet sich im [COLOR %s]/addon_data/%s/[/COLOR] Ordner[/COLOR]' % (COLOR2, COLOR1, ADDON_ID))
...
def asciiCheck(use=None, over=False):
	if use == None:
		source = DIALOG.browse(3, '[COLOR %s]Wahle den zu scannenden Ordner[/COLOR]' % COLOR2, 'files', '', False, False, HOME)
		if over == True:
			yes = 1
		else:
			yes = DIALOG.yesno(ADDONTITLE,'[COLOR %s]Willst du alle Dateinamen mit spezial Zeichen [COLOR %s]entfernen[/COLOR] oder willst du nur [COLOR %s]scannen[/COLOR] und das Ergebnis im Log anzeigen?[/COLOR]' % (COLOR2, COLOR1, COLOR1), yeslabel='[B][COLOR lime]JA, entfernen[/COLOR][/B]', nolabel='[B][COLOR red]NEIN, nur scannen[/COLOR][/B]')
	else: 
		source = use
		yes = 1
	...
	for base, dirs, files in os.walk(source):
		...
		for file in files:
			...
			DP.update(prog2,"[COLOR %s]Suche nach nicht ASCII files" % COLOR2,'[COLOR %s]%s[/COLOR]' % (COLOR1, d), 'Bitte Warten[/COLOR]')
			try:
				file.encode('ascii')
...
def backUpOptions(type, name=""):
	...
	try:
		...
		return
	if type == "build":
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du das aktuelle Build sichern?[/COLOR]" % COLOR2, nolabel="[B][COLOR red]NEIN, abbrechen[/COLOR][/B]", yeslabel="[B][COLOR lime]JA, Backup erstellen[/COLOR][/B]"):
			if name == "":
				name = getKeyboard("","Gib einen Namen ein fur die %s zip" % type)
			...
			for_progress  = 0
			ITEM          = []
			if not DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du den Addon_Data Ordner mit sichern?" % COLOR2, 'Dieser Beinhaltet [COLOR %s]alle[/COLOR] Addon Einstellungen inklusive Passwort und sonstige wichtige Informationen wie z.B. Skin-Verknupfungen. Es wird empfohlen nicht notwendige Addon_Data Ordner [COLOR %s]manuell[/COLOR] zu entfernen.' % (COLOR1, COLOR1), '[COLOR %s]%s[/COLOR] Addon_Data wird ignoriert[/COLOR]' % (COLOR1, ADDON_ID), yeslabel='[B][COLOR lime]JA, inkl. Daten[/COLOR][/B]',nolabel='[B][COLOR red]NEIN, ohne Daten[/COLOR][/B]'):
			...
			try:
				...
				try:
					...
					if DIALOG.yesno(ADDONTITLE, "[COLOR %s]In das aktuelle Verzeichnis kann nicht geschrieben werden, willst du das Verzeichnis wechseln?[/COLOR]" % COLOR2, yeslabel="[B][COLOR lime]JA, Verzeichnis wechseln[/COLOR][/B]", nolabel="[B][COLOR red]NEIN, abbrechen[/COLOR][/B]"):
						openS()
						return
					else:
						return
			DP.create("[COLOR %s]%s[/COLOR][COLOR %s]: Erstelle Zip[/COLOR]" % (COLOR1, ADDONTITLE,COLOR2), "[COLOR %s]Erstelle Backup Zip" % COLOR2, "", "Bitte Warten...[/COLOR]")
			for base, dirs, files in os.walk(HOME):
				dirs[:] = [d for d in dirs if d not in exclude_dirs]
				files[:] = [f for f in files if f not in exclude_files]
				for file in files:
			...
			for base, dirs, files in os.walk(HOME):
				...
				for file in files:
					try:
						...
						DP.update(int(progress), '[COLOR %s]Erstelle Backup Zip: [COLOR%s]%s[/COLOR] / [COLOR%s]%s[/COLOR]' % (COLOR2, COLOR1, for_progress, COLOR1, N_ITEM), '[COLOR %s]%s[/COLOR]' % (COLOR1, file), '')
			...
			zipf.close()
			xbmc.sleep(500)
			DP.update(100, "Erstelle %s_guisettings.zip" % name, "", "")
		...
		if name == "":
			guiname = getKeyboard("","Gib einen Namen ein fur die %s zip" % type)
			...
			try:
				...
				for fold in match:
					...
					if not fd in ['skin.confluence', 'skin.re-touch', 'skin.estuary', 'skin.estouchy']:
						if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du den folgenden Skin Ordner zur the GuiFix Zip Datei hinzufugen?[/COLOR]" % COLOR2, "[COLOR %s]%s[/COLOR]" % (COLOR1, fd), yeslabel="[B][COLOR lime]JA, Skin hinzufugen[/COLOR][/B]", nolabel="[B][COLOR red]NEIN, abbrechen[/COLOR][/B]"):
		...
		if name == "":
			DIALOG.ok(ADDONTITLE, "[COLOR %s]GuiFix Backup erfolgreich:[/COLOR]" % (COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, guizipname))
	elif type == "theme":
		if not DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Willst du ein Theme Backup erstellen?[/COLOR]" % COLOR2, yeslabel="[B][COLOR lime]JA, erstellen[/COLOR][/B]", nolabel="[B][COLOR red]NEIN, abbrechen[/COLOR][/B]"): LogNotify("Theme Backup", "Cancelled!"); return False
		if name == "":
			themename = getKeyboard("","Gib einen Namen ein fur die %s zip" % type)
		...
		try:
			if not SKIN == 'skin.confluence':
				...
				if len(match2) > 1:
					if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Willst du die Texture Datei mit sichern?[/COLOR]" % COLOR2, "[COLOR %s]%s[/COLOR]" % (COLOR1, SKIN), yeslabel="[B][COLOR lime]JA, sichern[/COLOR][/B]", nolabel="[B][COLOR red]NEIN, abbrechen[/COLOR][/B]"):
						skinfold = os.path.join(ADDONS, SKIN, 'media')
						match2 = glob.glob(os.path.join(skinfold,'*.xbt'))
						for xbt in match2:
							if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Willst du diese Texture Datei sichern [COLOR %s]%s[/COLOR]?" % (COLOR1, COLOR2, xbt.replace(skinfold, "")[1:]), "from [COLOR %s]%s[/COLOR][/COLOR]" % (COLOR1, SKIN), yeslabel="[B][COLOR lime]JA, sichern[/COLOR][/B]", nolabel="[B][COLOR red]NEIN, abbrechen[/COLOR][/B]"):
								fn  = xbt
								fn2 = fn.replace(HOME, "")
								zipf.write(fn, fn2, zipfile.ZIP_DEFLATED)
				else:
					for xbt in match2:
						if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Willst du diese Texture Datei sichern [COLOR %s]%s[/COLOR]?" % (COLOR2, COLOR1, xbt.replace(skinfold, "")[1:]), "from [COLOR %s]%s[/COLOR][/COLOR]" % (COLOR1, SKIN), yeslabel="[B][COLOR lime]JA, sichern[/COLOR][/B]", nolabel="[B][COLOR red]NEIN, abbrechen[/COLOR][/B]"):
							fn  = xbt
							fn2 = fn.replace(HOME, "")
							zipf.write(fn, fn2, zipfile.ZIP_DEFLATED)
				ad_skin = os.path.join(ADDOND, SKIN, 'settings.xml')
				if os.path.exists(ad_skin):
					if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Willst du die [COLOR %s]settings.xml[/COLOR] in [COLOR %s]/addon_data/[/COLOR] mit sichern?" % (COLOR2, COLOR1, COLOR1), "[COLOR %s]%s[/COLOR]"  % (COLOR1, SKIN), yeslabel="[B][COLOR lime]JA, sichern[/COLOR][/B]", nolabel="[B][COLOR red]NEIN, abbrechen[/COLOR][/B]"):
				...
				if 'script.skinshortcuts' in match:
					if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Willst du die [COLOR %s]settings.xml[/COLOR] von [COLOR %s]script.skinshortcuts[/COLOR] mit sichern?" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR lime]JA, sichern[/COLOR][/B]", nolabel="[B][COLOR red]NEIN, abbrechen[/COLOR][/B]"):
						for base, dirs, files in os.walk(os.path.join(ADDOND,'script.skinshortcuts')):
							files[:] = [f for f in files if f not in exclude_files]
							for file in files:
								fn = os.path.join(base, file)
								zipf.write(fn, fn[len(HOME):], zipfile.ZIP_DEFLATED)
			if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Willst du den [COLOR %s]Backgrounds[/COLOR] Ordner mit sichern?[/COLOR]" % (COLOR2, COLOR1), yeslabel="[B][COLOR lime]JA, sichern[/COLOR][/B]", nolabel="[B][COLOR red]NEIN, abbrechen[/COLOR][/B]"):
				fn = DIALOG.browse(0, 'Wahle einen Pfad fur die Backgrounds', 'files', '', True, False, HOME, False)
				if not fn == HOME:
				...
				if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Willst du [COLOR %s]%s[/COLOR] mit sichern?[/COLOR]" % (COLOR2, COLOR1, text), yeslabel="[B][COLOR lime]JA, sichern[/COLOR][/B]", nolabel="[B][COLOR red]NEIN, abbrechen[/COLOR][/B]"):
					zipf.write(os.path.join(DATABASE, text), '/userdata/Database/%s' % text, zipfile.ZIP_DEFLATED)
			if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Willst du Addons mit sichern?[/COLOR]" % (COLOR2), yeslabel="[B][COLOR lime]JA[/COLOR][/B]", nolabel="[B][COLOR red]NEIN, abbrechen[/COLOR][/B]"):
				...
				for folder in sorted(fold, key = lambda x: x):
					...
					if os.path.exists(xml):
						...
						if len(match) > 0:
							...
							addonfolds.append(foldername)
				if KODIV > 16:
					selected = DIALOG.multiselect("%s: Wahle die Addons die du zur Zip hinzufugen willst." % ADDONTITLE, addonnames)
				else:
					selected = []; choice = 0
					tempaddonnames = ["-- Klicke hier zum fortfahren --"] + addonnames
					while not choice == -1:
						choice = DIALOG.select("%s: Wahle die Addons die du zur Zip hinzufugen willst." % ADDONTITLE, tempaddonnames)
						if choice == -1: break
						elif choice == 0: break
						else: 
							choice2 = (choice-1)
					...
					for item in selected:
						for base, dirs, files in os.walk(os.path.join(ADDONS,addonfolds[item])):
							...
							for file in files:
								...
								fn = os.path.join(base, file)
								zipf.write(fn, fn[len(HOME):], zipfile.ZIP_DEFLATED)
			if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Willst du die [COLOR %s]guisettings.xml[/COLOR] mit sichern?[/COLOR]" % (COLOR2, COLOR1), yeslabel="[B][COLOR lime]JA, sichern[/COLOR][/B]", nolabel="[B][COLOR red]NEIN, abbrechen[/COLOR][/B]"):
				zipf.write(GUISETTINGS, '/userdata/guisettings.xml', zipfile.ZIP_DEFLATED)
		except Exception, e:
			zipf.close()
			log("[Back Up] Type = '%s': %s" % (type, str(e)), xbmc.LOGNOTICE)
			DIALOG.ok(ADDONTITLE, "[COLOR %s]%s[/COLOR][COLOR %s] theme zip fehlgeschlagen:[/COLOR]" % (COLOR1, themename, COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, str(e)))
			if not tempzipname == '':
		...
		if not tempzipname == '':
			...
			if success == 0:
				xbmcvfs.copy(tempzipname, zipname)
				xbmcvfs.delete(tempzipname)
		DIALOG.ok(ADDONTITLE, "[COLOR %s]%s[/COLOR][COLOR %s] theme zip erfolgreich:[/COLOR]" % (COLOR1, themename, COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, zipname))
	elif type == "addondata":
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du den aktuellen Addon_Data Ordner sichern?[/COLOR]" % COLOR2, nolabel="[B][COLOR red]NEIN, abbrechen[/COLOR][/B]", yeslabel="[B][COLOR lime]JA, Addon_Data sichern[/COLOR][/B]"):
			if name == "":
				name = getKeyboard("","Gib einen Namen ein fur die %s zip" % type)
				if not name: return False
...
def restoreLocal(type):
	...
	try:
		...
		DIALOG.ok(ADDONTITLE, "[COLOR %s]Fehler beim Erstellen des Backup Verzeichnis:[/COLOR]" % (COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, str(e)))
		return
	file = DIALOG.browse(1, '[COLOR %s]Wahle das Backup das du wiederherstellen willst[/COLOR]' % COLOR2, 'files', '.zip', False, False, mybuilds)
	...
	if file == "" or not file.endswith('.zip'):
		...
		return
	DP.create(ADDONTITLE,'[COLOR %s]Installiere lokales Backup' % COLOR2,'', 'Bitte Warten[/COLOR]')
	...
	try:
		...
		DP.update(0, '[COLOR %s]Zip kann aus dem aktuellen Verzeichnis nicht gelesen werden.' % COLOR2, 'Kopiere Datei nach Packages')
	...
	if int(errors) >= 1:
		yes=DIALOG.yesno(ADDONTITLE, '[COLOR %s][COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, fn), 'Fertig: [COLOR %s]%s%s[/COLOR] [Fehler:[COLOR %s]%s[/COLOR]]' % (COLOR1, percent, '%', COLOR1, errors), 'Willst du die Fehler sehen?[/COLOR]', nolabel='[B][COLOR red]NEIN, abbrechen[/COLOR][/B]',yeslabel='[B][COLOR lime]JA, zeige Fehler[/COLOR][/B]')
		if yes:
	...
	else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du [COLOR %s]KODI schliessen[/COLOR] oder das [COLOR %s]Profil neu laden[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR red]Profil neu laden[/COLOR][/B]", nolabel="[B][COLOR lime]KODI schliessen[/COLOR][/B]")
	if todo == 1: reloadFix()
	else: killxbmc(True)

def restoreExternal(type):
	source = DIALOG.browse(1, '[COLOR %s]Wahle das Backup das du wiederherstellen willst[/COLOR]' % COLOR2, 'files', '.zip', False, False)
	if source == "" or not source.endswith('.zip'):
		LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Extere Wiederherstellung: Abgebrochen[/COLOR]" % COLOR2)
		return
	...
	DP.create(ADDONTITLE,'[COLOR %s]Lade Zip Datei' % COLOR2,'', 'Bitte Warten[/COLOR]')
	if type == "gui": loc = USERDATA
	...
	DP.update(0,'Installiere externes Backup','', 'Bitte Warten')
	percent, errors, error = extract.all(file,loc,DP)
	fixmetas()
	clearS('build')
	DP.close()
	...
	lookandFeelData('save')
	if int(errors) >= 1:
		yes=DIALOG.yesno(ADDONTITLE, '[COLOR %s][COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, zname), 'Fertig: [COLOR %s]%s%s[/COLOR] [Fehler:[COLOR %s]%s[/COLOR]]' % (COLOR1, percent, '%', COLOR1, errors), 'Willst du die Fehler sehen?[/COLOR]', nolabel='[B][COLOR red]NEIN, abbrechen[/COLOR][/B]',yeslabel='[B][COLOR lime]Ja, zeige Fehler[/COLOR][/B]')
		if yes:
			TextBox(ADDONTITLE, error.replace('\t',''))
	setS('installed', 'true')
	setS('extract', str(percent))
	...
	else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du [COLOR %s]KODI schliessen[/COLOR] oder das [COLOR %s]Profil neu laden[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR red]Profile neu laden[/COLOR][/B]", nolabel="[B][COLOR lime]Kodi schliessen[/COLOR][/B]")
	if todo == 1: reloadFix()
	else: killxbmc(True)
...
def whiteList(do):
	...
	if   do == 'edit':
		...
		while not choice in [-1, 0]:
			choice = DIALOG.select("%s: Wahle Addons die auf die WhiteList sollen." % ADDONTITLE, tempaddonnames)
			if choice == -1: break
			elif choice == 0: break
			else: 
				choice2 = (choice-1)
		...
		list = whiteList('read')
		if len(list) > 0:
			msg = "Hier ist eine Liste deiner WhitList Addons (inklusive Abhangigkeiten) die nicht entfernt werden, wenn du einen Frischen Start machst oder der UserData Ordner von einem Build uberschrieben wird.[CR][CR]"
			for item in list:
				try: name, id, fold = item
				except Exception, e: log(str(e))
		...
		source = DIALOG.browse(1, '[COLOR %s]Wahle eine WhiteList Datei zum Importieren[/COLOR]' % COLOR2, 'files', '.txt', False, False, HOME)
		log(str(source))
		...
		with open(WHITELIST, 'a') as f:
			for item in lines:
				...
				if not id in idList:
					...
					f.write(writing)
			LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]%s Addons hinzugefugt[/COLOR]" % (COLOR2, count))
	elif do == 'export':
		source = DIALOG.browse(3, '[COLOR %s]Wahle wohin die WhitList exportiert werden soll[/COLOR]' % COLOR2, 'files', '.txt', False, False, HOME)
		log(str(source), xbmc.LOGDEBUG)
		try:
			xbmcvfs.copy(WHITELIST, os.path.join(source, 'whitelist.txt'))
			DIALOG.ok(ADDONTITLE, "[COLOR %s]WhiteList wurden exportiert nach:[/COLOR]" % (COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, os.path.join(source, 'whitelist.txt')))
			...
			if not DIALOG.yesno(ADDONTITLE, "[COLOR %s]In das aktuelle Verzeichnis kann nicht geschriebn werden, willst du das Verzeichnis wechseln?[/COLOR]" % COLOR2, yeslabel="[B][COLOR lime]JA, Verzeichnis wechseln[/COLOR][/B]", nolabel="[B][COLOR red]NEIN, abbrechen[/COLOR][/B]"):
				LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Whitelist Export Cancelled[/COLOR]" % (COLOR2, e))
			else:
				whitelist(export)
	elif do == 'clear':
		if not DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du die WhiteList bereinigen?" % COLOR2, "Dieser Prozess kann nicht ruckgangig gemacht werden.[/COLOR]", yeslabel="[B][COLOR lime]JA, bereinigen[/COLOR][/B]", nolabel="[B][COLOR red]NEIN, abbrechen[/COLOR][/B]"):
		...
		try:
			for root, dirs, files in os.walk(PACKAGES):
				...
				if file_count > 0:
					...
					if over: yes=1
					else: yes=DIALOG.yesno("[COLOR %s]Reinige Package Dateien" % COLOR2, "[COLOR %s]%s[/COLOR] Dateien gefunden / [COLOR %s]%s[/COLOR] Gesamtgrosse." % (COLOR1, str(file_count), COLOR1, size), "Willst du die Dateien bereinigen?[/COLOR]", nolabel='[B][COLOR red]NEIN, abbrechen[/COLOR][/B]',yeslabel='[B][COLOR lime]JA, Dateien reinigen[/COLOR][/B]')
					if yes:
						for f in files: os.unlink(os.path.join(root, f))
						for d in dirs: shutil.rmtree(os.path.join(root, d))
						LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE),'[COLOR %s]Reinige Packages: Erfolgreich![/COLOR]' % COLOR2)
				else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE),'[COLOR %s]Reinige Packages: Keine gefunden![/COLOR]' % COLOR2)
		except Exception, e:
			LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE),'[COLOR %s]Reinige Packages: Fehler![/COLOR]' % COLOR2)
...
def clearPackagesStartup():
	...
	if os.path.exists(PACKAGES):
		...
		try:
			...
			if file_count > 0: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Reinige Packages: Erfolgreich: %s[/COLOR]' % (COLOR2, convertSize(cleanupsize)))
			else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Reinige Packages: Keine gefunden![/COLOR]' % COLOR2)
		except Exception, e:
			LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Reinige Packages: Fehler![/COLOR]' % COLOR2)
			log("Clear Packages Error: %s" % str(e), xbmc.LOGERROR)
...
def clearCache(over=None):
	...
	LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Reinige Cache: Entfernte %s Dateien[/COLOR]' % (COLOR2, delfiles))
...
def checkSources():
	...
	if len(match) > 0:
		...
		if len(bad) > 0:
			choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]%s[/COLOR][COLOR %s] fehlerhafte Quellen wurden gefunden" % (COLOR1, len(bad), COLOR2),"Willst du alle oder einzel entfernen?[/COLOR]", yeslabel="[B][COLOR lime]Alle entfernen[/COLOR][/B]", nolabel="[B][COLOR red]Einzeln entfernen[/COLOR][/B]")
			if choice == 1:
				remove = bad
			else:
				for name, path, sharing, working in bad: 
					log("%s sources: %s, %s" % (name, path, working), xbmc.LOGNOTICE)
					if DIALOG.yesno(ADDONTITLE, "[COLOR %s]%s[/COLOR][COLOR %s] wurde als fehlerhaft gemeldet" % (COLOR1, name, COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, path), "[COLOR %s]%s[/COLOR]" % (COLOR1, working), yeslabel="[B][COLOR lime]Quelle entfernen[/COLOR][/B]", nolabel="[B][COLOR red]Quelle behalten[/COLOR][/B]"):
			...
			if len(remove) > 0:
				...
				removed = len(remove)
				DIALOG.ok(ADDONTITLE, "[COLOR %s]Scann auf fehlerhafte Quellen fertig" % COLOR2, "Funktonierend: [COLOR %s]%s[/COLOR] | Behalten: [COLOR %s]%s[/COLOR] | Entfernt: [COLOR %s]%s[/COLOR][/COLOR]" % (COLOR2, COLOR1, alive, COLOR1, kept, COLOR1, removed))
...
def killxbmc(over=None):
	if over: choice = 1
	else: choice = DIALOG.yesno('KODI schliessen', '[COLOR %s]Du bist dabei KODI zu schliessen' % COLOR2, 'Willst du fortfahrene?[/COLOR]', nolabel='[B][COLOR red]NEIN, abbrechen[/COLOR][/B]',yeslabel='[B][COLOR lime]JA, KODI schliessen[/COLOR][/B]')
	if choice == 1:
...
def reloadFix(default=None):
	DIALOG.ok(ADDONTITLE, "[COLOR %s]WARNUNG: Manchmal kann beim Profil neu laden KODI absturzen.  Solange das Profil neu geladen wird, bitte keine Tasten drucken![/COLOR]" % COLOR2)
	if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
	if default == None:
		lookandFeelData('save')
	redoThumbs()
...
def swapSkins(goto):
	...
	if xbmc.getCondVisibility("Window.isVisible(yesnodialog)"):
		ebi('SendClick(11)')
	else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Frische Installation: Skin Wechsel wird ausgefuhrt![/COLOR]' % COLOR2); return False
	xbmc.sleep(500)
...
def kodi17Fix():
	...
	if len(disabledAddons) > 0:
		x = 0
		DP.create(ADDONTITLE,'[COLOR %s]Aktiviere deaktivierte Addons' % COLOR2,'', 'Bitte Warten[/COLOR]')
		for item in disabledAddons:
			x += 1
			prog = int(percentage(x, len(disabledAddons)))
			DP.update(prog, "", "Aktiviert: [COLOR %s]%s[/COLOR]" % (COLOR1, item))
			addonDatabase(item, 1)
			if DP.iscanceled(): break
		if DP.iscanceled(): 
			DP.close()
			LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Aktivierung der Addons abgbrochen![/COLOR]" % COLOR2)
			sys.exit()
		DP.close()
	forceUpdate()
	ebi("ReloadSkin()")
...
def purgeDb(name):
	...
	LogNotify("[COLOR %s]Datenbank Reinigung[/COLOR]" % COLOR1, "[COLOR %s]%s Fertig[/COLOR]" % (COLOR2, show[len(show)-1]))
...
def oldThumbs():
	...
	if len(images) > 0: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Reinige Thumbs: %s Dateien / %s MB[/COLOR]!' % (COLOR2, str(len(images)), removed))
	else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Reinige Thumbs: Keine gefunden![/COLOR]' % COLOR2)
