...
def autoConfig(msg='', TxtColor='0xFFFFFFFF', Font='font12', BorderWidth=10):
	class MyWindow(xbmcgui.WindowDialog):
		...
		def __init__(self,msg='',L=0,T=0,W=1280,H=720,TxtColor='0xFFFFFFFF',Font='font12',BorderWidth=10):
			...
			header = '[COLOR %s]Advanced Settings Konfigurator[/COLOR]' % (COLOR2)
			...
			header = '[COLOR %s]Video Cache Grosse[/COLOR]' % (COLOR2)
			...
			msg3 = "[COLOR %s]Anzahl der Bytes zum Puffern im RAM.  Wird dies auf [COLOR %s]0[/COLOR] gesetzt, wird der Cache auf die Disk geschrieben anstatt in den RAM.  Info: Kodi braucht 3x soviel an freiem RAM des hier gewahlten Wertes. Das kann bei falscher Einstellung zum Abturz von KODI fuhren. (1/3 of Freier RAM: [COLOR %s]%s[/COLOR])[/COLOR]" % (COLOR2, COLOR1, COLOR1, freeMemory)
			...
			current1 = '[COLOR %s]Aktuell:[/COLOR] [COLOR %s]%s MB[/COLOR]' % (COLOR1, COLOR2, self.currentVideo)
			recommended1 = '[COLOR %s]Empfohlen:[/COLOR] [COLOR %s]%s MB[/COLOR]' % (COLOR1, COLOR2, self.recommendedVideo)
			...
			msg3 = "[COLOR %s][B]curlclienttimeout[/B] ist die Zeit in Sekunden bis libcurl Verbindungen in den timeout gehen und [B]curllowspeedtime[/B] ist die Zeit in Sekunden, die libcurl benotigt um eine Verbindung mit niedriger Geschwindigkeit zu prufen.  Bei einer langsamen Verbindung auf 20 setzten.[/COLOR]" % COLOR2
			...
			current2 = '[COLOR %s]Aktuell:[/COLOR] [COLOR %s]%ss[/COLOR]' % (COLOR1, COLOR2, self.currentCurl)
			recommended2 = '[COLOR %s]Empfohlen:[/COLOR] [COLOR %s]%ss[/COLOR]' % (COLOR1, COLOR2, self.recommendedCurl)
			...
			msg3 = "[COLOR %s]Der Wert dieser Einstellung ist ein Multiplikator der standard Einstellung. Wen KODI eine normale Bluray Raw Datei mit 36 Mbit/s lad, benotigt ein Wert von 2 eine Bandbreite von 72 Mbit/s. Hier kann im Vergleich zum RAM ein zu hoher Wert KODI nicht zum Absturz bringen.[/COLOR]" % COLOR2
			...
			current3 = '[COLOR %s]Aktuell:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR1, COLOR2, self.currentRead)
			recommended3 = '[COLOR %s]Empfohlen:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR1, COLOR2, self.recommendedRead)
			...
			self.addControl(self.Header4)
			msg4 = "[COLOR %s]Diese Einstellung zwingt KODI einen Cache fur alle Video Dateien (inklusive lokales Netzwerk, Internet und lokalen Festplatten) zu verwenden. Der standard Wert ist 0 und buffert nur Internet Quellen.[/COLOR]" % COLOR2
			...
			self.Button0 = xbmcgui.ControlRadioButton(rightside+(BorderWidth*3), B1, (W/2)-(BorderWidth*4), 30, '0: Buffer alle Internet Quellen', font='font12', focusTexture=radiobgfocus, noFocusTexture=radiobgnofocus, focusOnTexture=radiofocus, noFocusOnTexture=radiofocus, focusOffTexture=radionofocus, noFocusOffTexture=radionofocus)
			self.Button1 = xbmcgui.ControlRadioButton(rightside+(BorderWidth*3), B2, (W/2)-(BorderWidth*4), 30, '1: Buffer alle Quellen', font='font12', focusTexture=radiobgfocus, noFocusTexture=radiobgnofocus, focusOnTexture=radiofocus, noFocusOnTexture=radiofocus, focusOffTexture=radionofocus, noFocusOffTexture=radionofocus)
			self.Button2 = xbmcgui.ControlRadioButton(rightside+(BorderWidth*3), B3, (W/2)-(BorderWidth*4), 30, '2: Buffer nur richtige Internet Quellen', font='font12', focusTexture=radiobgfocus, noFocusTexture=radiobgnofocus, focusOnTexture=radiofocus, noFocusOnTexture=radiofocus, focusOffTexture=radionofocus, noFocusOffTexture=radionofocus)
			self.Button3 = xbmcgui.ControlRadioButton(rightside+(BorderWidth*3), B4, (W/2)-(BorderWidth*4), 30, '3: Kein Buffer', font='font12', focusTexture=radiobgfocus, noFocusTexture=radiobgnofocus, focusOnTexture=radiofocus, noFocusOnTexture=radiofocus, focusOffTexture=radionofocus, noFocusOffTexture=radionofocus)
			...
			self.buttonWrite=xbmcgui.ControlButton(leftside,T+H-40-BorderWidth,(W/2)-(BorderWidth*2),35,"Schreibe Datei",textColor="0xFF000000",focusedColor="0xFF000000",alignment=2,focusTexture=buttonfocus,noFocusTexture=buttonnofocus)
			self.buttonCancel=xbmcgui.ControlButton(rightside+BorderWidth*2,T+H-40-BorderWidth,(W/2)-(BorderWidth*2),35,"Schliessen",textColor="0xFF000000",focusedColor="0xFF000000",alignment=2,focusTexture=buttonfocus,noFocusTexture=buttonnofocus)
		...
		def updateCurrent(self, control):
			if control == self.videoCacheSize:
				...
				current = '[COLOR %s]Aktuell:[/COLOR] [COLOR %s]%s MB[/COLOR]' % (COLOR1, COLOR2, int(self.currentVideo))
				...
				current = '[COLOR %s]Aktuell:[/COLOR] [COLOR %s]%ss[/COLOR]' % (COLOR1, COLOR2, int(self.currentCurl))
				...
				current = '[COLOR %s]Aktuell:[/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR1, COLOR2, int(self.currentRead))
		...
		def doWrite(self):
			...
			if os.path.exists(ADVANCED):
				choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Es ist aktuell eine [COLOR %s]AdvancedSettings.xml[/COLOR] vorhanden, willst du diese entfernen und fortfahren?[/COLOR]" % (COLOR2, COLOR1), yeslabel="[B][COLOR lime]JA, entfernen[/COLOR][/B]", nolabel="[B][COLOR red]NEIN, abbrechen[/COLOR][/B]")
				if choice == 0: return
...
def apkInstaller(apk):
	...
	xbmc.executebuiltin('Skin.SetString(apkinstaller,  %s wurde herunter geladen.[CR]Klicke Installieren im nachsten Fenster!)' % apk)
...
def firstRun():
	class MyWindow(xbmcgui.WindowXMLDialog):
		def __init__(self, *args, **kwargs):
			...
			self.msg   = "Aktuell ist kein Build installiert von %s.\n\nWahle 'Build Menu' um ein Build zu installieren oder 'Ignorieren' um den Hinweis zu schliessen.\n\nDanke das du den  %s verwendest." % (ADDONTITLE, ADDONTITLE)
...
def updateWindow(name='Testing Window', current='1.0', new='1.1', icon=ICON, fanart=FANART):
	class MyWindow(xbmcgui.WindowXMLDialog):
		def __init__(self, *args, **kwargs):
			...
			self.fanart = kwargs['fanart']
			self.msgupdate  = "Es gibt ein Update des installierten Builds:\n[COLOR %s]%s[/COLOR]\n\nAktuelle Version: v[COLOR %s]%s[/COLOR]\nNeuste Version: v[COLOR %s]%s[/COLOR]\n\n[COLOR %s]*Empfohlen: Frische Installation[/COLOR]" % (COLOR1, self.name, COLOR1, self.current, COLOR1, self.new, COLOR1)
			self.msgcurrent = "Es ist die neuste Version des Builds installiert:\n[COLOR %s]%s[/COLOR]\n\nAktuelle Version: v[COLOR %s]%s[/COLOR]\nNeuste Version: v[COLOR %s]%s[/COLOR]\n\n[COLOR %s]*Empfohlen: Frische Installation[/COLOR]" % (COLOR1, self.name, COLOR1, self.current, COLOR1, self.new, COLOR1)
