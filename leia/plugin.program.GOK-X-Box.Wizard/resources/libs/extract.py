...
def allWithProgress(_in, _out, dp, ignore, title):
	...
	for item in zin.infolist():
		...
		skip = False
		line1  = '%s [COLOR %s][B][Fehler:%s][/B][/COLOR]' % (title, COLOR2, errors)
		line2  = '[COLOR %s][B]Datei:[/B][/COLOR] [COLOR %s]%s/%s[/COLOR] ' % (COLOR2, COLOR1, count, int(nFiles))
		line2 += '[COLOR %s][B]Grosse:[/B][/COLOR] [COLOR %s]%s/%s[/COLOR]' % (COLOR2, COLOR1, wiz.convertSize(size), zipsize)
	...
	if dp.iscanceled(): 
		dp.close()
		wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Entpacken abgebrochen[/COLOR]" % COLOR2)
		sys.exit()
