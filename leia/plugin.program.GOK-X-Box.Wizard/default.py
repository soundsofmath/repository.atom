...
#                                                                              #
#  Translated and modified by Buildstuben                                      #
#                                                                              #
################################################################################
...
INCLUDESHOWBOXARIZE = wiz.getS('includeshowboxarize')
...
def index():
	...
	addDir ('Instandhaltung'   ,'maint',    icon=ICONMAINT,    themeit=THEME1)
	...
	addDir ('Daten Sichern'     ,'savedata', icon=ICONSAVE,     themeit=THEME1)
	if HIDECONTACT == 'No': addFile('Kontakt' ,'contact', icon=ICONCONTACT,  themeit=THEME1)
	if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
	addFile('Einstellungen'      ,'settings', icon=ICONSETTINGS, themeit=THEME1)
	if DEVELOPER == 'true': addDir('Entwickler Menu','developer', icon=ICONSETTINGS, themeit=THEME1)
...
def buildMenu():
	...
	if not WORKINGURL == True:
		...
		addDir ('Daten Speichern Menu'       ,'savedata', icon=ICONSAVE,     themeit=THEME3)
		if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
		addFile('URL Text Datei nicht korrekt formatiert', '', icon=ICONBUILDS, themeit=THEME3)
		...
		if len(match) >= 1:
			if SEPERATE == 'true':
				...
				if count15 > 0:
					...
					addFile('[B]%s Isengard Builds(%s)[/B]' % (state, count15), 'togglesetting',  'show15', themeit=THEME3)
					if SHOW15 == 'true':
						for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
							if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
							if not DEVELOPER == 'true' and wiz.strTest(name): continue
			...
			if adultcount > 0:
				addFile('Aktuell nur Adult Builds vorhanden', '', icon=ICONBUILDS, themeit=THEME3)
				addFile('Adults Builds im Menu anzeigen > Misc', '', icon=ICONBUILDS, themeit=THEME3)
			else:
				addFile('Aktuell keine Builds %s' % ADDONTITLE, '', icon=ICONBUILDS, themeit=THEME3)
		else: addFile('Text Datei der Builds nicht korrekt formatiert.', '', icon=ICONBUILDS, themeit=THEME3)
	setView('files', 'viewType')

def viewBuild(name):
	WORKINGURL = wiz.workingURL(BUILDFILE)
	if not WORKINGURL == True:
		addFile('Url der TXT Datei nicht richtig formatiert', '', themeit=THEME3)
		addFile('%s' % WORKINGURL, '', themeit=THEME3)
		return
	if wiz.checkBuild(name, 'version') == False: 
		addFile('Fehler beim Lesen der TXT Datei.', '', themeit=THEME3)
		addFile('%s wurde in der Build Liste nicht gefunden.' % name, '', themeit=THEME3)
		return
	...
	for version, url, gui, kodi, themefile, icon, fanart, preview, adult, description in match:
		...
		if not preview == "http://": addFile('Zeige Video Vorschau', 'buildpreview', name, description=description, fanart=fanart, icon=icon, themeit=THEME3)
		...
		if warning == True:
			addFile('[I]Build fur Kodi Version %s(installiert: %s)[/I]' % (str(kodi), str(KODIV)), '', fanart=fanart, icon=icon, themeit=THEME3)
		addFile(wiz.sep('INSTALLIEREN'), '', fanart=fanart, icon=icon, themeit=THEME3)
		addFile('Frische Installation'   , 'install', name, 'fresh'  , description=description, fanart=fanart, icon=icon, themeit=THEME1)
		addFile('Standard Installation', 'install', name, 'normal' , description=description, fanart=fanart, icon=icon, themeit=THEME1)
...
def viewThirdList(number):
	...
	if not work == True:
		addFile('URL der TXT Datei nicht richtig formatiert', '', icon=ICONBUILDS, themeit=THEME3)
		addFile('%s' % WORKINGURL, '', icon=ICONBUILDS, themeit=THEME3)
	else:
...
def editThirdParty(number):
	name  = eval('THIRD%sNAME' % number)
	url   = eval('THIRD%sURL' % number)
	name2 = wiz.getKeyboard(name, 'Wizard Name eintragen')
	url2  = wiz.getKeyboard(url, 'Wizard TXT Datei URL eintragen')
...
def apkScraper(name=""):
	if name == 'kodi':
		...
		addFile("Offizielle Kodi Apk\'s", themeit=THEME1)
		rc = False
...
def apkMenu(url=None):
	if url == None:
		addDir ('Offizielle Kodi Apk\'s', 'apkscrape', 'kodi', icon=ICONAPK, themeit=THEME1)
		if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
	if not APKFILE == 'http://':
		if url == None:
		...
		if APKWORKING == True:
			...
			if len(match) > 0:
				...
				for name, section, url, icon, fanart, adult, description in match:
					...
					if x < 1:
						addFile("Aktuell keine APKs vorhanden!", '', themeit=THEME2)
			...
			addFile('URL Text Datei nicht korrekt formatiert', '', themeit=THEME3)
			...
			if len(match) > 0:
				for name, url, repository, repositoryxml, repositoryurl, icon, fanart, adult, description in match:
					if os.path.exists(os.path.join(ADDONS, plugin)):
						...
						selected = DIALOG.select("[COLOR %s]Addon bereits installiert , was willst du gerne machen?[/COLOR]" % COLOR2, do)
						if selected == 0:
							...
							if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du die Addon Daten entfernen:" % COLOR2, "[COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR1, plugin), yeslabel="[B][COLOR lime]JA, Entfernen[/COLOR][/B]", nolabel="[B][COLOR red]NEIN, abbrechen[/COLOR][/B]"):
					...
					if not repository.lower() == 'none' and not os.path.exists(repo):
						...
						if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du die Repository installieren [COLOR %s]%s[/COLOR]:" % (COLOR2, COLOR1, plugin), "[COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR1, repository), yeslabel="[B][COLOR lime]JA, installieren[/COLOR][/B]", nolabel="[B][COLOR red]NEIN, abbrechen[/COLOR][/B]"): 
							...
							if len(ver) > 0:
								...
								wiz.log("[Addon Installer] Repository nicht installiert: Kann nicht auf die URL zugreifen! (%s)" % repository)
						else: wiz.log("[Addon Installer] Repository fur %s nicht installiert: %s" % (plugin, repository))
...
def installAddon(name, url):
	if not wiz.workingURL(url) == True: wiz.LogNotify("[COLOR %s]Addon Installer[/COLOR]" % COLOR1, '[COLOR %s]%s:[/COLOR] [COLOR %s]Zip URL nicht korrekt![/COLOR]' % (COLOR1, name, COLOR2)); return
	if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
	DP.create(ADDONTITLE,'[COLOR %s][B]Download:[/B][/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, name), '', '[COLOR %s]Bitte Warten[/COLOR]' % COLOR2)
	urlsplit = url.split('/')
	...
	DP.update(0, title,'', '[COLOR %s]Bitte Warten[/COLOR]' % COLOR2)
	percent, errors, error = extract.all(lib,ADDONS,DP, title=title)
	DP.update(0, title,'', '[COLOR %s]Installiere Abhangigkeiten[/COLOR]' % COLOR2)
	installed(name)
	installDep(name, DP)
	DP.close()
	wiz.ebi('UpdateAddonRepos()')
...
def maintMenu(view=None):
	...
	wizlogsize = ': [COLOR red]Not Found[/COLOR]' if not os.path.exists(WIZLOG) else ": [COLOR lime]%s[/COLOR]" % wiz.convertSize(os.path.getsize(WIZLOG))
	if includeall == 'true':
		includelas = 'true'
		includecov = 'true'
		includeexo = 'true'
		...
		includepla = 'true' if INCLUDEPLACENTA == 'true' else 'false'
		includegen = 'true' if INCLUDEGENESISREBORN   == 'true' else 'false'
		includesho = 'true' if INCLUDESHOWBOXARIZE == 'true' else 'false'
	...
	feq        = ['Immer', 'Taglich', 'Alle 3 Tage', 'Jede Woche']
	addDir ('[B]Reinigung Tools[/B]'       ,'maint', 'clean',  icon=ICONMAINT, themeit=THEME1)
	if view == "clean" or SHOWMAINT == 'true': 
		addFile('Komplette Reinigung: [COLOR lime][B]%s[/B][/COLOR]' % wiz.convertSize(totalsize)  ,'fullclean',       icon=ICONMAINT, themeit=THEME3)
		addFile('Cache reinigen: [COLOR lime][B]%s[/B][/COLOR]' % wiz.convertSize(sizecache)     ,'clearcache',      icon=ICONMAINT, themeit=THEME3)
		addFile('Packages reinigen: [COLOR lime][B]%s[/B][/COLOR]' % wiz.convertSize(sizepack)   ,'clearpackages',   icon=ICONMAINT, themeit=THEME3)
		addFile('Thumbnails reinigen: [COLOR lime][B]%s[/B][/COLOR]' % wiz.convertSize(sizethumb),'clearthumb',      icon=ICONMAINT, themeit=THEME3)
		addFile('Alte Thumbnails reinigen', 'oldThumbs',      icon=ICONMAINT, themeit=THEME3)
		addFile('Crash Logs reinigen',               'clearcrash',      icon=ICONMAINT, themeit=THEME3)
		addFile('Datenbank reinigen',                'purgedb',         icon=ICONMAINT, themeit=THEME3)
		addFile('[COLOR red]Frischer Start[/COLOR]',                    'freshstart',      icon=ICONMAINT, themeit=THEME3)
	...
	if view == "addon" or SHOWMAINT == 'true': 
		...
		addFile('Aktiviere/Deaktiviere XXX Addons',    'toggleadult',     icon=ICONMAINT, themeit=THEME3)
		addFile('Suche Addon Updates',            'forceupdate',     icon=ICONMAINT, themeit=THEME3)
		addFile('Verstecke Passwort in der Keyboard Eingabe',   'hidepassword',   icon=ICONMAINT, themeit=THEME3)
		addFile('Zeige Passwort in der Keyboard Eingabe', 'unhidepassword', icon=ICONMAINT, themeit=THEME3)
	addDir ('[B]Diverse Wartungsoptionen[/B]'     ,'maint', 'misc',   icon=ICONMAINT, themeit=THEME1)
	if view == "misc" or SHOWMAINT == 'true': 
		addFile('Kodi 17 Fix',                    'kodi17fix',       icon=ICONMAINT, themeit=THEME3)
		addFile('Skin neu laden',                    'forceskin',       icon=ICONMAINT, themeit=THEME3)
		addFile('Profil neu laden',                 'forceprofile',    icon=ICONMAINT, themeit=THEME3)
		...
		addFile('Zeige Fehler in der Log: %s' % (errorsfound), 'viewerrorlog',    icon=ICONMAINT, themeit=THEME3)
		...
		addFile('Reinige Wizard Log Datei%s' % wizlogsize,'clearwizlog',     icon=ICONMAINT, themeit=THEME3)
	addDir ('[B]Sichern/Wiederherstellen[/B]'     ,'maint', 'backup',   icon=ICONMAINT, themeit=THEME1)
	if view == "backup" or SHOWMAINT == 'true':
		addFile('Reinige Sicherungs-Ordner',        'clearbackup',     icon=ICONMAINT,   themeit=THEME3)
		addFile('Sicherungs-Pfad: [COLOR %s]%s[/COLOR]' % (COLOR2, MYBUILDS),'settings', 'Maintenance', icon=ICONMAINT, themeit=THEME3)
		addFile('[Sichern]: Build',               'backupbuild',     icon=ICONMAINT,   themeit=THEME3)
		addFile('[Sichern]: GuiFix',              'backupgui',       icon=ICONMAINT,   themeit=THEME3)
		addFile('[Sichern]: Theme',               'backuptheme',     icon=ICONMAINT,   themeit=THEME3)
		addFile('[Sichern]: Addon_Daten',          'backupaddon',     icon=ICONMAINT,   themeit=THEME3)
		addFile('[Wiederherstellen]: Lokales Build',         'restorezip',      icon=ICONMAINT,   themeit=THEME3)
		addFile('[Wiederherstellen]: Lokaler GuiFix',        'restoregui',      icon=ICONMAINT,   themeit=THEME3)
		addFile('[Wiederherstellen]: Lokale Addon_Daten',    'restoreaddon',    icon=ICONMAINT,   themeit=THEME3)
		addFile('[Wiederherstellen]: Externes Build',      'restoreextzip',   icon=ICONMAINT,   themeit=THEME3)
		addFile('[Wiederherstellen]: Externer GuiFix',     'restoreextgui',   icon=ICONMAINT,   themeit=THEME3)
		addFile('[Wiederherstellen]: Externe Addon_Daten', 'restoreextaddon', icon=ICONMAINT,   themeit=THEME3)
	...
	if view == "tweaks" or SHOWMAINT == 'true': 
		if not ADVANCEDFILE == 'http://' and not ADVANCEDFILE == '':
			...
			if os.path.exists(ADVANCED):
				addFile('Zeige aktuelle AdvancedSettings.xml',   'currentsettings', icon=ICONMAINT, themeit=THEME3)
				addFile('Entferne aktuelle AdvancedSettings.xml', 'removeadvanced',  icon=ICONMAINT, themeit=THEME3)
			addFile('Schnelle Einstellung der AdvancedSettings.xml',    'autoadvanced',    icon=ICONMAINT, themeit=THEME3)
		addFile('Duchsuche Quellen auf Fehler',  'checksources',    icon=ICONMAINT, themeit=THEME3)
		addFile('Suche defekte Repositories',   'checkrepos',      icon=ICONMAINT, themeit=THEME3)
		addFile('Fix Addons ohne Update',        'fixaddonupdate',  icon=ICONMAINT, themeit=THEME3)
	...
	addFile('Zeige alle Instandhaltungs-Optionen: %s' % maint.replace('true',on).replace('false',off) ,'togglesetting', 'showmaint', icon=ICONMAINT, themeit=THEME2)
	addDir ('[I]<< Zuruck zum Hauptmenu[/I]', icon=ICONMAINT, themeit=THEME2)
	addFile('Drittanbieter Wizards: %s' % thirdparty.replace('true',on).replace('false',off) ,'togglesetting', 'enable3rd', fanart=FANART, icon=ICONMAINT, themeit=THEME1)
	if thirdparty == 'true':
		...
		addFile('Bearbeite Drittanbieter Wizard 1: [COLOR %s]%s[/COLOR]' % (COLOR2, first), 'editthird', '1', icon=ICONMAINT, themeit=THEME3)
		addFile('Bearbeite Drittanbieter Wizard 2: [COLOR %s]%s[/COLOR]' % (COLOR2, secon), 'editthird', '2', icon=ICONMAINT, themeit=THEME3)
		addFile('Bearbeite Drittanbieter Wizard 3: [COLOR %s]%s[/COLOR]' % (COLOR2, third), 'editthird', '3', icon=ICONMAINT, themeit=THEME3)
	addFile('Auto Reinigung', '', fanart=FANART, icon=ICONMAINT, themeit=THEME1)
	addFile('Auto Reinigung beim Start: %s' % autoclean.replace('true',on).replace('false',off) ,'togglesetting', 'autoclean',   icon=ICONMAINT, themeit=THEME3)
	if autoclean == 'true':
		addFile('--- Auto Reinigung Intervall: [B][COLOR lime]%s[/COLOR][/B]' % feq[AUTOFEQ], 'changefeq', icon=ICONMAINT, themeit=THEME3)
		addFile('--- Reinige Cache beim Start: %s' % cache.replace('true',on).replace('false',off), 'togglesetting', 'clearcache', icon=ICONMAINT, themeit=THEME3)
		addFile('--- Reinige Packages beim Start: %s' % packages.replace('true',on).replace('false',off), 'togglesetting', 'clearpackages', icon=ICONMAINT, themeit=THEME3)
		addFile('--- Reinige alte Thumbs beim Start: %s' % thumbs.replace('true',on).replace('false',off), 'togglesetting', 'clearthumbs', icon=ICONMAINT, themeit=THEME3)
	addFile('Reinige Video Cache', '', fanart=FANART, icon=ICONMAINT, themeit=THEME1)
	addFile('Video Cache mit einbeziehen beim Cache reinigen: %s' % includevid.replace('true',on).replace('false',off), 'togglecache', 'includevideo', icon=ICONMAINT, themeit=THEME3)
	if includevid == 'true':
		addFile('--- Alle Video Addons mit einbeziehen: %s' % includeall.replace('true',on).replace('false',off), 'togglecache', 'includeall', icon=ICONMAINT, themeit=THEME3)
		addFile('--- Lastship mit einbeziehen: %s' % includelas.replace('true',on).replace('false',off), 'togglecache', 'includelastship', icon=ICONMAINT, themeit=THEME3)
		addFile('--- Covenant mit einbeziehen: %s' % includecov.replace('true',on).replace('false',off), 'togglecache', 'includecovenant', icon=ICONMAINT, themeit=THEME3)
		addFile('--- Exodus mit einbeziehen: %s' % includeexo.replace('true',on).replace('false',off), 'togglecache', 'includeexodus', icon=ICONMAINT, themeit=THEME3)
		addFile('--- Flixnet mit einbeziehen: %s' % includefli.replace('true',on).replace('false',off), 'togglecache', 'includeflixnet', icon=ICONMAINT, themeit=THEME3)
		addFile('--- Incursion mit einbeziehen: %s' % includeinc.replace('true',on).replace('false',off), 'togglecache', 'includeincursion', icon=ICONMAINT, themeit=THEME3)
		addFile('--- Placenta mit einbeziehen: %s' % includepla.replace('true',on).replace('false',off), 'togglecache', 'includeplacenta', icon=ICONMAINT, themeit=THEME3)
		addFile('--- GenesisReborn mit einbeziehen: %s' % includegen.replace('true',on).replace('false',off), 'togglecache', 'includegenesisreborn', icon=ICONMAINT, themeit=THEME3)
		addFile('--- ShowBoxArize mit einbeziehen: %s' % includesho.replace('true',on).replace('false',off), 'togglecache', 'includeshowboxarize', icon=ICONMAINT, themeit=THEME3)
		addFile('--- Gurzil mit einbeziehen: %s' % includegur.replace('true',on).replace('false',off), 'togglecache', 'includegurzil', icon=ICONMAINT, themeit=THEME3)
...
def advancedWindow(url=None):
	if not ADVANCEDFILE == 'http://':
		...
		addFile('Schnelles Konfigurieren der AdvancedSettings.xml', 'autoadvanced', icon=ICONMAINT, themeit=THEME3)
		if os.path.exists(ADVANCED): 
			addFile('Zeige aktuelle AdvancedSettings.xml', 'currentsettings', icon=ICONMAINT, themeit=THEME3)
			addFile('Entferne aktuelle AdvancedSettings.xml', 'removeadvanced',  icon=ICONMAINT, themeit=THEME3)
...
def writeAdvanced(name, url):
	ADVANCEDWORKING = wiz.workingURL(url)
	if ADVANCEDWORKING == True:
		if os.path.exists(ADVANCED): choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du deine aktuelle AdvancedSettings ersetzen [COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, name), yeslabel="[B][COLOR lime]JA, Ersetzen[/COLOR][/B]", nolabel="[B][COLOR red]NEIN, Abbrechen[/COLOR][/B]")
		else: choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du herunter laden und installieren [COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, name), yeslabel="[B][COLOR lime]Installieren[/COLOR][/B]", nolabel="[B][COLOR red]Abbrechen[/COLOR][/B]")
		...
		if choice == 1:
			...
			f.write(file)
			f.close()
			DIALOG.ok(ADDONTITLE, '[COLOR %s]AdvancedSettings.xml wurde erfolgreich geschrieben.  Klicke OK um KODI zum schliessen zu zwingen.[/COLOR]' % COLOR2)
			wiz.killxbmc(True)
		else: wiz.log("[Advanced Settings] Installieren abgebrochen"); wiz.LogNotify('[COLOR %s]%s[/COLOR]' % (COLOR1, ADDONTITLE), "[COLOR %s]Schreiben abgebrochen![/COLOR]" % COLOR2); return
...
def saveMenu():
	on = '[COLOR lime]AN[/COLOR]'; off = '[COLOR red]AUS[/COLOR]'
	...
	addDir ('Behalte Trakt Data',               'trakt',                icon=ICONTRAKT, themeit=THEME1)
	addDir ('Behalte Real Debrid',              'realdebrid',           icon=ICONREAL,  themeit=THEME1)
	addFile('Importiere Speicher Daten',              'managedata', 'import', icon=ICONSAVE,  themeit=THEME1)
	addFile('Exportiere Speicher Daten',              'managedata', 'export', icon=ICONSAVE,  themeit=THEME1)
	addFile('- Hier Klicken um Einstellungen frei zu schalten -', '', themeit=THEME3)
	addFile('Speichere Trakt: %s' % trakt.replace('true',on).replace('false',off)                       ,'togglesetting', 'keeptrakt',      icon=ICONTRAKT, themeit=THEME1)
	addFile('Speichere Real Debrid: %s' % real.replace('true',on).replace('false',off)                  ,'togglesetting', 'keepdebrid',     icon=ICONREAL,  themeit=THEME1)
	addFile('Behalte \'Sources.xml\': %s' % sources.replace('true',on).replace('false',off)           ,'togglesetting', 'keepsources',    icon=ICONSAVE,  themeit=THEME1)
	addFile('Behalte \'Profiles.xml\': %s' % profiles.replace('true',on).replace('false',off)         ,'togglesetting', 'keepprofiles',   icon=ICONSAVE,  themeit=THEME1)
	addFile('Behalte \'Advancedsettings.xml\': %s' % advanced.replace('true',on).replace('false',off) ,'togglesetting', 'keepadvanced',   icon=ICONSAVE,  themeit=THEME1)
	addFile('Behalte \'Favourites.xml\': %s' % favourites.replace('true',on).replace('false',off)     ,'togglesetting', 'keepfavourites', icon=ICONSAVE,  themeit=THEME1)
	addFile('Behalte Super Favourites: %s' % super.replace('true',on).replace('false',off)            ,'togglesetting', 'keepsuper',      icon=ICONSAVE,  themeit=THEME1)
	addFile('Behalte installierte Repo\'s: %s' % repos.replace('true',on).replace('false',off)           ,'togglesetting', 'keeprepos',      icon=ICONSAVE,  themeit=THEME1)
	addFile('Behalte meine \'WhiteList\': %s' % whitelist.replace('true',on).replace('false',off)        ,'togglesetting', 'keepwhitelist',  icon=ICONSAVE,  themeit=THEME1)
	if whitelist == 'true':
		...
		addFile('Reinige meine Whitelist',       'whitelist', 'clear',  icon=ICONSAVE,  themeit=THEME1)
...
def traktMenu():
	trakt = '[COLOR lime]AN[/COLOR]' if KEEPTRAKT == 'true' else '[COLOR red]AUS[/COLOR]'
	last = str(TRAKTSAVE) if not TRAKTSAVE == '' else 'Trakt wurde noch nicht gespeichert.'
	addFile('[I]Registeriere dich gratis auf http://trakt.tv[/I]', '', icon=ICONTRAKT, themeit=THEME3)
	addFile('Speichere Trakt Daten: %s' % trakt, 'togglesetting', 'keeptrakt', icon=ICONTRAKT, themeit=THEME3)
	...
	for trakt in traktit.ORDER:
		...
		else:                        addFile('[COLOR lime]Addon Daten: %s[/COLOR]' % auser,'authtrakt', trakt, icon=icon, fanart=fanart, menu=menu)
		if user == "":
			if os.path.exists(file): addFile('[COLOR red]Gespeicherte Daten: Gespeicherte Datei gefunden(importiere Daten)[/COLOR]','importtrakt', trakt, icon=icon, fanart=fanart, menu=menu2)
			else :                   addFile('[COLOR red]Gespeicherte Daten: Nicht gespeichert[/COLOR]','savetrakt', trakt, icon=icon, fanart=fanart, menu=menu2)
		else:                        addFile('[COLOR lime]Gespeicherte Daten: %s[/COLOR]' % user, '', icon=icon, fanart=fanart, menu=menu2)
	
	if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
	addFile('Speichere alle Trakt Daten',          'savetrakt',    'all', icon=ICONTRAKT,  themeit=THEME3)
	addFile('Gespeicherte Trakt Daten wiederherstellen', 'restoretrakt', 'all', icon=ICONTRAKT,  themeit=THEME3)
	addFile('Importiere Trakt Daten',            'importtrakt',  'all', icon=ICONTRAKT,  themeit=THEME3)
	addFile('Reinige alle gespeicherten Trakt Daten',   'cleartrakt',   'all', icon=ICONTRAKT,  themeit=THEME3)
	addFile('Reinige alle Addon Daten',         'addontrakt',   'all', icon=ICONTRAKT,  themeit=THEME3)
	setView('files', 'viewType')

def realMenu():
	real = '[COLOR lime]AN[/COLOR]' if KEEPREAL == 'true' else '[COLOR red]AUS[/COLOR]'
	last = str(REALSAVE) if not REALSAVE == '' else 'Real Debrid wurde noch nicht gespeichert.'
	addFile('[I]http://real-debrid.com ist ein Bezahl-Dienst.[/I]', '', icon=ICONREAL, themeit=THEME3)
	addFile('Speichere Real Debrid Daten: %s' % real, 'togglesetting', 'keepdebrid', icon=ICONREAL, themeit=THEME3)
	...
	for debrid in debridit.ORDER:
		...
		addFile('[+]-> %s' % name,     '', icon=icon, fanart=fanart, themeit=THEME3)
		if not os.path.exists(path): addFile('[COLOR red]Addon Daten: Nicht installiert[/COLOR]', '', icon=icon, fanart=fanart, menu=menu)
		elif not auser:              addFile('[COLOR red]Addon Daten: Nicht registriert[/COLOR]','authdebrid', debrid, icon=icon, fanart=fanart, menu=menu)
		else:                        addFile('[COLOR lime]Addon Daten: %s[/COLOR]' % auser,'authdebrid', debrid, icon=icon, fanart=fanart, menu=menu)
		if user == "":
			if os.path.exists(file): addFile('[COLOR red]Gespeicherte Daten: Gespeicherte Datei gefunden(importiere Daten)[/COLOR]','importdebrid', debrid, icon=icon, fanart=fanart, menu=menu2)
			else :                   addFile('[COLOR red]Gespeicherte Daten: Nicht gespeichert[/COLOR]','savedebrid', debrid, icon=icon, fanart=fanart, menu=menu2)
		else:                        addFile('[COLOR lime]Gespeicherte Daten: %s[/COLOR]' % user, '', icon=icon, fanart=fanart, menu=menu2)
	
	if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
	addFile('Speichere alle Real Debrid Daten',          'savedebrid',    'all', icon=ICONREAL,  themeit=THEME3)
	addFile('Gespeicherte Real Debrid Daten wiederherstellen', 'restoredebrid', 'all', icon=ICONREAL,  themeit=THEME3)
	addFile('Importiere Real Debrid Daten',            'importdebrid',  'all', icon=ICONREAL,  themeit=THEME3)
	addFile('Reinige alle gespeicherten Real Debrid Daten',   'cleardebrid',   'all', icon=ICONREAL,  themeit=THEME3)
	addFile('Reinige alle Addon Daten',               'addondebrid',   'all', icon=ICONREAL,  themeit=THEME3)
	setView('files', 'viewType')

def loginMenu():
	login = '[COLOR lime]AN[/COLOR]' if KEEPLOGIN == 'true' else '[COLOR red]AUS[/COLOR]'
...
def removeAddonMenu():
	...
	for folder in sorted(fold, key = lambda x: x):
		...
		if os.path.exists(xml):
			...
			try: 
				...
				pass
	if len(addonnames) == 0:
		wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Keine Addons zum entfernen[/COLOR]" % COLOR2)
		return
	if KODIV > 16:
		selected = DIALOG.multiselect("%s: Wahle das Addon das du entfernen willst." % ADDONTITLE, addonnames)
	else:
		selected = []; choice = 0
		tempaddonnames = ["-- Hier klicken um fort zu fahren --"] + addonnames
		while not choice == -1:
			choice = DIALOG.select("%s: Wahle das Addon das du entfernen willst." % ADDONTITLE, tempaddonnames)
			if choice == -1: break
			elif choice == 0: break
			else: 
				choice2 = (choice-1)
	...
	if len(selected) > 0:
		...
		else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du [COLOR %s]KODI schliessen[/COLOR] oder [COLOR %s]Profil neu laden[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR lime]Profil neu laden[/COLOR][/B]", nolabel="[B][COLOR red]KODI schliessen[/COLOR][/B]")
...
def removeAddonDataMenu():
	if os.path.exists(ADDOND):
		addFile('[COLOR red][B][Entferne][/B][/COLOR] Alle Addon_Daten', 'removedata', 'all', themeit=THEME2)
		addFile('[COLOR red][B][Entferne][/B][/COLOR] Alle Addon_Daten der deinstallierten Addons', 'removedata', 'uninstalled', themeit=THEME2)
		addFile('[COLOR red][B][Entferne][/B][/COLOR] Alle leeren Ordner in den Addon_Daten', 'removedata', 'empty', themeit=THEME2)
		addFile('[COLOR red][B][Entferne][/B][/COLOR] %s Addon_Daten' % ADDONTITLE, 'resetaddon', themeit=THEME2)
		if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
		...
		for folder in sorted(fold, key = lambda x: x):
			...
			if foldername in EXCLUDES: folderdisplay = '[COLOR lime][B][Gesichert][/B][/COLOR] %s' % folderdisplay
			else: folderdisplay = '[COLOR red][B][Entfernen][/B][/COLOR] %s' % folderdisplay
			addFile(' %s' % folderdisplay, 'removedata', foldername, icon=icon, fanart=fanart, themeit=THEME2)
	else:
		addFile('Kein Addon_Daten Ordner gefunden.', '', themeit=THEME3)
	setView('files', 'viewType')

def enableAddons():
	addFile("[I][B][COLOR red]!!Hinweis: Die Deaktivierung mancher Addons kann Probleme verursachen!![/COLOR][/B][/I]", '', icon=ICONMAINT)
	fold = glob.glob(os.path.join(ADDONS, '*/'))
	x = 0
	for folder in sorted(fold, key = lambda x: x):
		foldername = os.path.split(folder[:-1])[1]
		...
		if os.path.exists(addonxml):
			...
			try:
				...
				state  = "[COLOR lime][Aktiviert][/COLOR]"
				goto   = "false"
			except:
...
def changeFeq():
	feq        = ['Bei jedem Start', 'Jeden Tag', 'Alle 3 Tage', 'Jede Woche']
	change     = DIALOG.select("[COLOR %s]Wie oft willst du die automatische Reinigung beim Start?[/COLOR]" % COLOR2, feq)
	if not change == -1: 
		wiz.setS('autocleanfeq', str(change))
		wiz.LogNotify('[COLOR %s]Automatische Reinigung[/COLOR]' % COLOR1, '[COLOR %s]Intervall jetzt %s[/COLOR]' % (COLOR2, feq[change]))

def developer():
	addFile('Konvertiere Text Daten zu 0.1.7',         'converttext',           themeit=THEME1)
	addFile('Ertstelle QR Code',                      'createqr',              themeit=THEME1)
	addFile('Teste Benachrichtigung',                  'testnotify',            themeit=THEME1)
	addFile('Teste Update',                         'testupdate',            themeit=THEME1)
	addFile('Teste erster Start',                      'testfirst',             themeit=THEME1)
	addFile('Teste erster Start Einstellungen',             'testfirstrun',          themeit=THEME1)
	addFile('Teste APk',             'testapk',          themeit=THEME1)
...
def buildWizard(name, type, theme=None, over=False):
	if over == False:
		...
		if testbuild == False:
			wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Build wurde nicht gefunden[/COLOR]" % COLOR2)
			return
	...
	if type == 'gui':
		if name == BUILDNAME:
			if over == True: yes = 1
			else: yes = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Willst du den GuiFix anwenden:' % COLOR2, '[COLOR %s]%s[/COLOR]?[/COLOR]' % (COLOR1, name), nolabel='[B][COLOR red]NEIN, Abbrechen[/COLOR][/B]',yeslabel='[B][COLOR lime]JA, Fix anwenden[/COLOR][/B]')
		else: 
			yes = DIALOG.yesno("%s - [COLOR red]WARNUNG!![/COLOR]" % ADDONTITLE, "[COLOR %s][COLOR %s]%s[/COLOR] Build ist aktuell nicht installiert." % (COLOR2, COLOR1, name), "Willst du den GuiFix trotzdem anwenden?.[/COLOR]", nolabel='[B][COLOR red]NEIN, Abbrechen[/COLOR][/B]',yeslabel='[B][COLOR lime]JA, Fix anwenden[/COLOR][/B]')
		if yes:
			buildzip = wiz.checkBuild(name,'gui')
			zipname = name.replace('\\', '').replace('/', '').replace(':', '').replace('*', '').replace('?', '').replace('"', '').replace('<', '').replace('>', '').replace('|', '')
			if not wiz.workingURL(buildzip) == True: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]GuiFix: Inkorrekte Zip Url![/COLOR]' % COLOR2); return
			if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
			DP.create(ADDONTITLE,'[COLOR %s][B]Download GuiFix:[/B][/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, name),'', 'Bitte Warten')
			...
			xbmc.sleep(500)
			title = '[COLOR %s][B]Installiere:[/B][/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, name)
			DP.update(0, title,'', 'Bitte Warten')
			...
			else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Der GuiFix wurde installiert.  Willst du das Profil neu laden oder KODI schliessen?[/COLOR]" % COLOR2, yeslabel="[B][COLOR red]Profil neu laden[/COLOR][/B]", nolabel="[B][COLOR lime]KODI schliessen[/COLOR][/B]")
			if todo == 1: wiz.reloadFix()
			else: DIALOG.ok(ADDONTITLE, "[COLOR %s]Um die Anderungen zu speichern musst du KODi jetzt schliessen, klicke OK um KODI zu schliessen[/COLOR]" % COLOR2); wiz.killxbmc('true')
		else:
			wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]GuiFix: Abgebrochen![/COLOR]' % COLOR2)
	elif type == 'fresh':
		freshStart(name)
	elif type == 'normal':
		if url == 'normal':
		...
		if warning == True:
			yes_pressed = DIALOG.yesno("%s - [COLOR red]WARNUNG!![/COLOR]" % ADDONTITLE, '[COLOR %s]Es besteht die Moglichkeit das es Darstellungsprobleme des Skins gibt' % COLOR2, 'Bei der Installation eines %s Builds auf einer KODI %s Installation' % (wiz.checkBuild(name, 'kodi'), KODIV), 'Willst du trotzdem installieren: [COLOR %s]%s v%s[/COLOR]?[/COLOR]' % (COLOR1, name, wiz.checkBuild(name,'version')), nolabel='[B][COLOR red]NEIN, Abbrechen[/COLOR][/B]',yeslabel='[B][COLOR lime]JA, Installieren[/COLOR][/B]')
		else:
			if not over == False: yes_pressed = 1
			else: yes_pressed = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Willst du herunterladen und installieren:' % COLOR2, '[COLOR %s]%s v%s[/COLOR]?[/COLOR]' % (COLOR1, name, wiz.checkBuild(name,'version')), nolabel='[B][COLOR red]NEIN, Abbrechen[/COLOR][/B]',yeslabel='[B][COLOR lime]JA, Installieren[/COLOR][/B]')
		if yes_pressed:
			wiz.clearS('build')
			buildzip = wiz.checkBuild(name, 'url')
			zipname = name.replace('\\', '').replace('/', '').replace(':', '').replace('*', '').replace('?', '').replace('"', '').replace('<', '').replace('>', '').replace('|', '')
			if not wiz.workingURL(buildzip) == True: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Build Installation: Inkorrekte Zip Url![/COLOR]' % COLOR2); return
			if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
			DP.create(ADDONTITLE,'[COLOR %s][B]Download:[/B][/COLOR] [COLOR %s]%s v%s[/COLOR]' % (COLOR2, COLOR1, name, wiz.checkBuild(name,'version')),'', 'Bitte Warten')
			...
			if int(float(percent)) > 0:
				...
				if int(float(errors)) > 0:
					yes=DIALOG.yesno(ADDONTITLE, '[COLOR %s][COLOR %s]%s v%s[/COLOR]' % (COLOR2, COLOR1, name, wiz.checkBuild( name,'version')), 'Erledigt: [COLOR %s]%s%s[/COLOR] [Fehler:[COLOR %s]%s[/COLOR]]' % (COLOR1, percent, '%', COLOR1, errors), 'Willst du die Fehler sehen?[/COLOR]', nolabel='[B][COLOR red]NEIN Danke[/COLOR][/B]', yeslabel='[B][COLOR lime]Zeige die Fehler[/COLOR][/B]')
					if yes:
				...
				else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du [COLOR %s]KODI schliessen[/COLOR] oder [COLOR %s]Profil neu laden[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR red]Profil neu laden[/COLOR][/B]", nolabel="[B][COLOR lime]KODI schliessen[/COLOR][/B]")
				if todo == 1: wiz.reloadFix()
		...
		if theme == None:
			...
			if not themefile == 'http://' and wiz.workingURL(themefile) == True:
				...
				if len(themelist) > 0:
					if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Diese Build [COLOR %s]%s[/COLOR] gibt es mit [COLOR %s]%s[/COLOR] verschiedenen Themen" % (COLOR2, COLOR1, name, COLOR1, len(themelist)), "Willst du es jetzt installieren?[/COLOR]", yeslabel="[B][COLOR lime]Installiere Theme[/COLOR][/B]", nolabel="[B][COLOR red]Abbrechen[/COLOR][/B]"):
						wiz.log("Theme List: %s " % str(themelist))
						ret = DIALOG.select(ADDONTITLE, themelist)
						wiz.log("Theme Installation gewahlt: %s" % ret)
		...
		else: installtheme = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Willst du das Theme installieren:' % COLOR2, '[COLOR %s]%s[/COLOR]' % (COLOR1, theme), 'fur [COLOR %s]%s v%s[/COLOR]?[/COLOR]' % (COLOR1, name, wiz.checkBuild(name,'version')), yeslabel="[B][COLOR lime]Installiere Theme[/COLOR][/B]", nolabel="[B][COLOR red]Abbrechen[/COLOR][/B]")
		if installtheme:
			themezip = wiz.checkTheme(name, theme, 'url')
			zipname = name.replace('\\', '').replace('/', '').replace(':', '').replace('*', '').replace('?', '').replace('"', '').replace('<', '').replace('>', '').replace('|', '')
			if not wiz.workingURL(themezip) == True: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Theme Installation: Inkorrekte Zip Url![/COLOR]' % COLOR2); return False
			if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
			DP.create(ADDONTITLE,'[COLOR %s][B]Download:[/B][/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, theme),'', 'Bitte Warten')
			lib=os.path.join(PACKAGES, '%s.zip' % zipname)
			try: os.remove(lib)
			except: pass
			...
			if url not in ["fresh", "normal"]:
				...
				if test == True:
					...
					#if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Installing the theme [COLOR %s]%s[/COLOR] requires the skin to be swaped back to [COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, theme, COLOR1, skin[5:]), "Would you like to switch the skin?[/COLOR]", yeslabel="[B][COLOR lime]Switch Skin[/COLOR][/B]", nolabel="[B][COLOR red]Don't Switch[/COLOR][/B]"):
					...
					else: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Theme Installation: Skin wechsel Timed Out![/COLOR]' % COLOR2); return
					xbmc.sleep(500)
...
def thirdPartyInstall(name, url):
	if not wiz.workingURL(url):
		LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Inkorrekte URL fur Build[/COLOR]' % COLOR2); return
	type = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du eine [COLOR %s]Frische Installation[/COLOR] oder eine [COLOR %s]Normale Installation[/COLOR] for:[/COLOR]" % (COLOR2, COLOR1, COLOR1), "[COLOR %s]%s[/COLOR]" % (COLOR1, name), yeslabel="[B][COLOR lime]Frische Installation[/COLOR][/B]", nolabel="[B][COLOR red]Normale Installation[/COLOR][/B]")
	if type == 1:
		freshStart('third', True)
	wiz.clearS('build')
	zipname = name.replace('\\', '').replace('/', '').replace(':', '').replace('*', '').replace('?', '').replace('"', '').replace('<', '').replace('>', '').replace('|', '')
	if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
	DP.create(ADDONTITLE,'[COLOR %s][B]Download:[/B][/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, name),'', 'Please Wait')
	lib=os.path.join(PACKAGES, '%s.zip' % zipname)
	try: os.remove(lib)
	except: pass
	downloader.download(url, lib, DP)
	xbmc.sleep(500)
	title = '[COLOR %s][B]Installiert:[/B][/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, name)
	DP.update(0, title,'', 'Bitte Warten')
	percent, errors, error = extract.all(lib,HOME,DP, title=title)
	if int(float(percent)) > 0:
		wiz.fixmetas()
		...
		wiz.setS('extract', str(percent))
		wiz.setS('errors', str(errors))
		wiz.log('INSTALLIERT %s: [ERRORS:%s]' % (percent, errors))
		try: os.remove(lib)
		except: pass
		if int(float(errors)) > 0:
			yes=DIALOG.yesno(ADDONTITLE, '[COLOR %s][COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, name), 'Erledigt: [COLOR %s]%s%s[/COLOR] [Fehler:[COLOR %s]%s[/COLOR]]' % (COLOR1, percent, '%', COLOR1, errors), 'Willst du die Fehler sehen?[/COLOR]', nolabel='[B][COLOR red]NEIN, Danke[/COLOR][/B]',yeslabel='[B][COLOR lime]Zeige Fehler[/COLOR][/B]')
			if yes:
				if isinstance(errors, unicode):
					error = error.encode('utf-8')
				wiz.TextBox(ADDONTITLE, error)
...
def apkInstaller(apk, url):
	...
	if wiz.platform() == 'android':
		yes = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du herunterladen und installieren:" % COLOR2, "[COLOR %s]%s[/COLOR]" % (COLOR1, apk), yeslabel="[B][COLOR lime]Download[/COLOR][/B]", nolabel="[B][COLOR red]Abbrechen[/COLOR][/B]")
		if not yes: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Fehler: Installation abgebrochen[/COLOR]' % COLOR2); return
		display = apk
		if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
		if not wiz.workingURL(url) == True: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]APK Installer: Inkorrekte Apk Url![/COLOR]' % COLOR2); return
		DP.create(ADDONTITLE,'[COLOR %s][B]Download:[/B][/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, display),'', 'Bitte Warten')
	...
	else: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]FEHLER: Keine Android Hardware[/COLOR]' % COLOR2)
...
def createMenu(type, add, name):
	if   type == 'saveaddon':
		...
		menu_items.append((THEME3 % 'Speichere %s Daten' % add3,               'RunPlugin(plugin://%s/?mode=save%s&name=%s)' %    (ADDON_ID, add2, name2)))
		menu_items.append((THEME3 % 'Wiederherstellung %s Daten' % add3,            'RunPlugin(plugin://%s/?mode=restore%s&name=%s)' % (ADDON_ID, add2, name2)))
		menu_items.append((THEME3 % 'Reinige %s Daten' % add3,              'RunPlugin(plugin://%s/?mode=clear%s&name=%s)' %   (ADDON_ID, add2, name2)))
	elif type == 'save'    :
		menu_items=[]
		add2  = urllib.quote_plus(add.lower().replace(' ', ''))
		add3  = add.replace('Debrid', 'Real Debrid')
		...
		menu_items.append((THEME3 % 'Reinige Addon %s Daten' % add3,        'RunPlugin(plugin://%s/?mode=addon%s&name=%s)' %   (ADDON_ID, add2, name2)))
	elif type == 'install'  :
		menu_items=[]
		name2 = urllib.quote_plus(name)
		menu_items.append((THEME2 % name,                                'RunAddon(%s, ?mode=viewbuild&name=%s)'  % (ADDON_ID, name2)))
		menu_items.append((THEME3 % 'Frische Installation',                     'RunPlugin(plugin://%s/?mode=install&name=%s&url=fresh)'  % (ADDON_ID, name2)))
		menu_items.append((THEME3 % 'Normale Installation',                    'RunPlugin(plugin://%s/?mode=install&name=%s&url=normal)' % (ADDON_ID, name2)))
		menu_items.append((THEME3 % 'GuiFix anwenden',                      'RunPlugin(plugin://%s/?mode=install&name=%s&url=gui)'    % (ADDON_ID, name2)))
		menu_items.append((THEME3 % 'Build Informationen',                 'RunPlugin(plugin://%s/?mode=buildinfo&name=%s)'  % (ADDON_ID, name2)))
	menu_items.append((THEME2 % '%s Einstellungen' % ADDONTITLE,              'RunPlugin(plugin://%s/?mode=settings)' % ADDON_ID))
	return menu_items

def toggleCache(state):
	cachelist = ['includevideo', 'includeall', 'includelastship', 'includecovenant', 'includeexodus', 'includeflixnet', 'includegenesisreborn', 'includeshowboxarize', 'includegurzil', 'includeplacenta', 'includeincursion']
	titlelist = ['Include Video Addons', 'Include All Addons', 'Include Lastship', 'Include Covenant', 'Include Exodus', 'Include Flixnet', 'Include GenesisReborn', 'Include ShowBoxArize', 'Include Gurzil', 'Include Placenta', 'Include Incursion']
	if state in ['true', 'false']:
		...
		if not state in ['includevideo', 'includeall'] and wiz.getS('includeall') == 'true':
			try:
				...
				DIALOG.ok(ADDONTITLE, "[COLOR %s]Du willst [COLOR %s]alle Video-Addons[/COLOR] deaktivieren[/COLOR] [COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, COLOR1, item))
			except:
				wiz.LogNotify("[COLOR %s]Toggle Cache[/COLOR]" % COLOR1, "[COLOR %s]Inkorrekte ID: %s[/COLOR]" % (COLOR2, state))
		else:
...
def errorChecking(log=None, count=None, all=None):
	if log == None:
		...
		oldlog  = wiz.Grab_Log(True, True)
		if not oldlog == False and not mainlog == False:
			which = DIALOG.select(ADDONTITLE, ["Zeige %s: %s Fehler" % (mainlog.replace(LOG, ""), errorChecking(mainlog, True, True)), "Zeige %s: %s Fehler" % (oldlog.replace(LOG, ""), errorChecking(oldlog, True, True))])
			if which == -1: wiz.LogNotify('[COLOR %s]Zeige Log[/COLOR]' % COLOR1, '[COLOR %s]Zeige Log abgebrochen![/COLOR]' % COLOR2); return
		elif mainlog == False and oldlog == False:
			wiz.LogNotify('[COLOR %s]Zeige Log[/COLOR]' % COLOR1, '[COLOR %s]Keine Log Datei gefunden![/COLOR]' % COLOR2)
			return
	...
	if log == False:
		if count == None:
			wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Keine Log Datei gefunden[/COLOR]" % COLOR2)
			return False
		else: 
			return 0
	else:
			...
			if len(match) > 0:
				...
				for item in match:
					if all == None and not ADDON_ID in item: continue
						...
						x += 1
						msg += "[COLOR red]Fehler Nummer %s[/COLOR]\n(PythonToCppException) : -->Python callback/script returned the following error<--%s-->End of Python script error report<--\n\n" % (x, item.replace('                                          ', '\n').replace('\\\\','\\').replace(HOME, ''))
				if x > 0:
					wiz.TextBox(ADDONTITLE, msg)
				else: wiz.LogNotify(ADDONTITLE, "Keine Fehler gefunden im Log")
			else: wiz.LogNotify(ADDONTITLE, "Keine Fehler gefunden im Log")
		else: wiz.LogNotify(ADDONTITLE, "Log Datei nicht gefunden")
...
def removeAddon(addon, name, over=False):
	if not over == False:
		...
		yes = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Willst du das Addon wirklich entfernen:'% COLOR2, 'Name: [COLOR %s]%s[/COLOR]' % (COLOR1, name), 'ID: [COLOR %s]%s[/COLOR][/COLOR]' % (COLOR1, addon), yeslabel='[B][COLOR lime]JA, entferne Addon[/COLOR][/B]', nolabel='[B][COLOR red]NEIN, abbrechen[/COLOR][/B]')
	if yes == 1:
...
def removeAddonData(addon, name=None, over=False):
	if addon == 'all':
		if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Willst du [COLOR %s]ALLE[/COLOR] Addon-Daten entfernen die im User_Data Ordner gespeichert sind?[/COLOR]' % (COLOR2, COLOR1), yeslabel='[B][COLOR lime]JA, entferne Daten[/COLOR][/B]', nolabel='[B][COLOR red]NEIN, abbrechen[/COLOR][/B]'):
		...
		if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Willst du [COLOR %s]ALLE[/COLOR] Addon-Daten entfernen die im User-Data Ordner gespeichert sind?[/COLOR]' % (COLOR2, COLOR1), yeslabel='[B][COLOR lime]JA, entferne Daten[/COLOR][/B]', nolabel='[B][COLOR red]NEIN, abbrechen[/COLOR][/B]'):
			total = 0
			...
			wiz.LogNotify('[COLOR %s]Reinige deinstallierte[/COLOR]' % COLOR1, '[COLOR %s]%s Ordner entfernt[/COLOR]' % (COLOR2, total))
		else: wiz.LogNotify('[COLOR %s]Entferne Addon Daten[/COLOR]' % COLOR1, '[COLOR %s]Abgebrochen![/COLOR]' % COLOR2)
	elif addon == 'empty':
		if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Willst du [COLOR %s]ALLE[/COLOR] leeren Ordner im User_Data Ordner entfernen?[/COLOR]' % (COLOR2, COLOR1), yeslabel='[B][COLOR lime]JA, entferne Ordner[/COLOR][/B]', nolabel='[B][COLOR red]NEIN, abbrechen[/COLOR][/B]'):
		...
		else: wiz.LogNotify('[COLOR %s]REntferne leere Ordner[/COLOR]' % COLOR1, '[COLOR %s]Abgebrochen![/COLOR]' % COLOR2)
	else:
		addon_data = os.path.join(USERDATA, 'addon_data', addon)
		if addon in EXCLUDES:
			wiz.LogNotify("[COLOR %s]Gesichertes Plugin[/COLOR]" % COLOR1, "[COLOR %s]Entfernen der Addon_Daten nicht erlaubt[/COLOR]" % COLOR2)
		elif os.path.exists(addon_data):  
			if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Willst du auch die Addon-Daten entfernen von:[/COLOR]' % COLOR2, '[COLOR %s]%s[/COLOR]' % (COLOR1, addon), yeslabel='[B][COLOR lime]JA, entferne Daten[/COLOR][/B]', nolabel='[B][COLOR red]NEIN, abbrechen[/COLOR][/B]'):
				wiz.cleanHouse(addon_data)
				try:
					shutil.rmtree(addon_data)
				except:
					wiz.log("Fehler beim Entfernen: %s" % addon_data)
			else: 
				wiz.log('Addon-Daten von %s wurden nicht entfernt' % addon)
	wiz.refresh()
...
def manageSaveData(do):
	if do == 'import':
		...
		source = DIALOG.browse(1, '[COLOR %s]Wahle den Ordner der SaveData.zip[/COLOR]' % COLOR2, 'files', '.zip', False, False, HOME)
		if not source.endswith('.zip'):
			wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Importieren fehlgeschlagen![/COLOR]" % (COLOR2))
			return
		...
		if os.path.exists(trakt):
			...
			for item in files:
				...
				if os.path.exists(old):
					if not DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du die aktuelle [COLOR %s]%s[/COLOR] Datei ersetzen?" % (COLOR2, COLOR1, item), yeslabel="[B][COLOR lime]JA, ersetzen[/COLOR][/B]", nolabel="[B][COLOR red]NEIN, abbrechen[/COLOR][/B]"): continue
					else: os.remove(old)
		...
		os.remove(tempfile)
		if x == 0: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Importieren fehlgeschlagen[/COLOR]" % COLOR2)
		else: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Importieren erfolgreich[/COLOR]" % COLOR2)
	elif do == 'export':
		...
		source = DIALOG.browse(3, '[COLOR %s]Wahle wohin du die SaveData.zip exportieren willst.[/COLOR]' % COLOR2, 'files', '', False, True, HOME)
		...
		zipf.close()
		if source == mybuilds:
			DIALOG.ok(ADDONTITLE, "[COLOR %s]SaveData.zip wurde gespeichert nach:[/COLOR]" % (COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, tempzip))
		else:
			try:
				xbmcvfs.copy(tempzip, os.path.join(source, 'SaveData.zip'))
				DIALOG.ok(ADDONTITLE, "[COLOR %s]SaveData.zip wurde gespeichert nach:[/COLOR]" % (COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, os.path.join(source, 'SaveData.zip')))
			except:
				DIALOG.ok(ADDONTITLE, "[COLOR %s]SaveData.zip wurde gespeichert nach:[/COLOR]" % (COLOR2), "[COLOR %s]%s[/COLOR]" % (COLOR1, tempzip))
...
def freshStart(install=None, over=False):
	...
	elif install == 'restore': yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du KODI resetten" % COLOR2, "auf standard Einstellungen", "bevor du das lokale Backup installierst?[/COLOR]", nolabel='[B][COLOR red]NEIN, abbrechen[/COLOR][/B]', yeslabel='[B][COLOR lime]JA, weiter[/COLOR][/B]')
	elif install: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du KODI resetten" % COLOR2, "auf standard Einstellungen", "bevor du [COLOR %s]%s[/COLOR] installierst?" % (COLOR1, install), nolabel='[B][COLOR red]NEIN, abbrechen[/COLOR][/B]', yeslabel='[B][COLOR lime]JA, weiter[/COLOR][/B]')
	else: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Willst du KODI resetten" % COLOR2, "auf standard Einstellungen?[/COLOR]", nolabel='[B][COLOR red]NEIN, abbrechen[/COLOR][/B]', yeslabel='[B][COLOR lime]JA, weiter[/COLOR][/B]')
	if yes_pressed:
		if not wiz.currSkin() in ['skin.confluence', 'skin.estuary']:
			skin = 'skin.confluence' if KODIV < 17 else 'skin.estuary'
			#yes=DIALOG.yesno(ADDONTITLE, "[COLOR %s]The skin needs to be set back to [COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, skin[5:]), "Before doing a fresh install to clear all Texture files,", "Would you like us to do that for you?[/COLOR]", yeslabel="[B][COLOR lime]Switch Skins[/COLOR][/B]", nolabel="[B][COLOR red]I'll Do It[/COLOR][/B]";
			#if yes:
			skinSwitch.swapSkins(skin)
			x = 0
			xbmc.sleep(1000)
			...
			if xbmc.getCondVisibility("Window.isVisible(yesnodialog)"):
				wiz.ebi('SendClick(11)')
			else: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Frische Installation: Skin Wechsel Timed Out![/COLOR]' % COLOR2); return False
			xbmc.sleep(1000)
		if not wiz.currSkin() in ['skin.confluence', 'skin.estuary']:
			wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Frische Installation: Skin Wechsel fehlgeschlagen![/COLOR]' % COLOR2)
			return
		wiz.addonUpdates('set')
		xbmcPath=os.path.abspath(HOME)
		DP.create(ADDONTITLE,"[COLOR %s]Berechne Dateien und Ordner" % COLOR2,'', 'Bitte Warten![/COLOR]')
		total_files = sum([len(files) for r, d, files in os.walk(xbmcPath)]); del_file = 0
		DP.update(0, "[COLOR %s]Checke Excludes Liste." % COLOR2)
		...
		DP.update(0, "[COLOR %s]Bereinige Dateien und Ordner:" % COLOR2)
		...
		for root, dirs, files in os.walk(xbmcPath,topdown=True):
			...
			if DP.iscanceled(): 
				DP.close()
				wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Frischer Start abgebrochen[/COLOR]" % COLOR2)
				return False
		for root, dirs, files in os.walk(xbmcPath,topdown=True):
			dirs[:] = [d for d in dirs if d not in EXCLUDES]
			for name in dirs:
				DP.update(100, '', 'Bereinige leere Ordner: [COLOR %s]%s[/COLOR]' % (COLOR1, name), '')
		...
		if not install == 'restore':
			wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Frische Installation: Abgebrochen![/COLOR]' % COLOR2)
			wiz.refresh()
...
####THANKS GUYS @ NaN #######
def clearCache():
	if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Willst du den Cache reinigen?[/COLOR]' % COLOR2, nolabel='[B][COLOR red]NEIN, abbrechen[/COLOR][/B]', yeslabel='[B][COLOR lime]JA, reinigen[/COLOR][/B]'):
		wiz.clearCache()
		#clearThumb()

def totalClean():
	if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Willst du den Cache, Packages und Thumbnails reinigen?[/COLOR]' % COLOR2, nolabel='[B][COLOR red]NEIN, abbrechen[/COLOR][/B]',yeslabel='[B][COLOR lime]JA, alle reinigen[/COLOR][/B]'):
		wiz.clearCache()
...
def clearThumb(type=None):
	...
	else: choice = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Willst du %s und Thumbnails Ordner entfernen?' % (COLOR2, latest), "Dieser wird beim Neustart wieder hergestellt[/COLOR]", nolabel='[B][COLOR red]NEIN, abbrechen[/COLOR][/B]', yeslabel='[B][COLOR lime]JA, entfernen[/COLOR][/B]')
	if choice == 1:
...
def purgeDb():
	...
	if KODIV >= 16: 
		choice = DIALOG.multiselect("[COLOR %s]Wahle DB Datei zum Reinigen[/COLOR]" % COLOR2, display)
		if choice == None: wiz.LogNotify("[COLOR %s]Reinige DB[/COLOR]" % COLOR1, "[COLOR %s]Abgebrochen[/COLOR]" % COLOR2)
		elif len(choice) == 0: wiz.LogNotify("[COLOR %s]Reinige DB[/COLOR]" % COLOR1, "[COLOR %s]Abgebrochen[/COLOR]" % COLOR2)
		else: 
			for purge in choice: wiz.purgeDb(DB[purge])
	else:
		choice = DIALOG.select("[COLOR %s]Wahle DB Datei zum Reinigen[/COLOR]" % COLOR2, display)
		if choice == -1: wiz.LogNotify("[COLOR %s]Reinige DB[/COLOR]" % COLOR1, "[COLOR %s]Abgebrochen[/COLOR]" % COLOR2)
...
def testnotify():
	...
	if url == True:
		try:
			...
			if id == False: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Hinweis: Nicht korrekt formatiert[/COLOR]" % COLOR2); return
			notify.notification(msg, True)
		except Exception, e:
			wiz.log("Error on Notifications Window: %s" % str(e), xbmc.LOGERROR)
	else: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Hinweis URL nicht korrekt[/COLOR]" % COLOR2)
