...
class Sources():
	...
	def process_results(self, results):
		...
		if not self.background:
			...
			if results:
				...
				try: self.progress_dialog.update_results_count(self.sources_sd, self.sources_720p, self.sources_1080p, self.sources_4k, self.sources_total, '', 0)
	...
	def play_file(self, results, source={}):
		...
		try:
			...
			if [i for i in items if i['scrape_provider'] == 'easynews']:
				items_with_retries = []
				for item in  items:
					items_with_retries.append(item)
					if item['scrape_provider'] == 'easynews':
						for count in range(1, self.easynews_max_retries + 1):
							item = dict(item)
							item['retry_count'] = count
							items_with_retries.append(item)
				items = list(items_with_retries)
			...
			for count, item in enumerate(items, 1):
				...
				try:
					...
					if 'retry_count' in item: text = ('%02d. [B]%s (RETRYx%s)[/B]' % (count, provider, item['retry_count']), item['display_name'].upper())
					else: text = ('%02d. [B]%s[/B]'% (count, provider), item['display_name'].upper())
					...
					if url:
						...
						while playback_successful is None:
							if self.progress_dialog.iscanceled() or monitor.abortRequested(): return self.playback_failed_action()
