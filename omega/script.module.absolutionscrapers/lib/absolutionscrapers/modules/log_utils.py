...
DEBUGPREFIX = '[ absolutionScrapers {0} | DEBUG ]'.format(version)
INFOPREFIX = '[ absolutionScrapers | INFO ]'
LOGPATH = control.transPath('special://logpath/')
log_file = os.path.join(LOGPATH, 'absolution.log')
debug_enabled = control.addon('plugin.video.absolution').getSetting('addon.debug')
...
def log(msg, trace=0):
    ...
    try:
        ...
        try:
            xbmc.log('absolutionScrapers Logging Failure: %s' % e, LOGDEBUG)
        except:
            pass
