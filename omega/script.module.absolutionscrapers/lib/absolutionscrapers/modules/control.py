...
addonObject = addon('script.module.absolutionscrapers')
...
def autoTraktSubscription(tvshowtitle, year, imdb, tvdb):
    from absolutionscrapers.modules import libtools
...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.absolution.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.absolution.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.absolution.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.absolution.artwork)')
...
# def metaFile():
    # if condVisibility('System.HasAddon(script.absolution.metadata)'):
        # return os.path.join(xbmcaddon.Addon('script.absolution.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
