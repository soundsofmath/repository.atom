...
try:
	...
	# Fen has changed the way it imports external modules.
	# It now ignores the addon.xml from CoCoScrapers and Python will therefore not know the module "from orion ...".
	# Add the Orion path manually to still be able to import the modules.
	import sys, os
	import xbmcaddon
	sys.path.append(os.path.join(xbmcaddon.Addon('script.module.orion').getAddonInfo('path'), 'lib'))
	from orion import *
	from orion.modules.oriontools import *
	from orion.modules.orionnetworker import *
...
class Orionoid:
	...
	priority = 0
	pack_capable = False # coco requires separate functions to scrape packs. Just retrieve packs as part of the normal scrape and label them as packs.
	hasMovies = True
	hasEpisodes = True
	...
	Keys = {
		'default' : 'VFhsQ1NFbEZXV2RWYVVKVVNVWkZaMUpUUVRWSlJGbG5VVk5CTVVsR1kyZFZRMEpNU1VSaloxUkRRa2hKUmtsblZrTkNSVWxGVFdkVVEwRTBTVVZOWjFGNVFreEpSVFJuVlVOQ1ZVbEZjMmRTVTBKTA==',
	...
	def __init__(self, type):
		self.type = type
		self.addon = xbmcaddon.Addon('script.module.cocoscrapers')
	...
	def _error(self):
		...
		xbmc.log('COCO SCRAPERS ORION [ERROR]: ' + parameters, xbmc.LOGERROR)
		...
		try:
			...
			for data in results:
				try:
					...
					elif self.type == Orionoid.TypeTorrent: continue # Umbrella fails if a torrent does not have a hash.
