...
class Orionoid:
	...
	priority = 0
	pack_capable = False # Chains Genocide requires separate functions to scrape packs. Just retrieve packs as part of the normal scrape and label them as packs.
	hasMovies = True
	hasEpisodes = True
	...
	def __init__(self, type):
		self.type = type
		self.addon = xbmcaddon.Addon('plugin.video.chainsgenocide')
		...
		self.cacheData = None
		self.key = 'VTNsQk1rbEZVV2RUZVVKT1NVVlpaMUpwUWs1SlJWRm5UbE5DUlVsRldXZE9RMEpXU1VWSloxTnBRVEpKUkd0blYwTkJOVWxFVldkVFEwSkpTVVpGWjFONVFrbEpSRTFuVkdsQ1NVbEZWV2RXUTBKRg=='
	...
	def _error(self):
		...
		xbmc.log('CHAINS GENOCIDE ORION [ERROR]: ' + parameters, xbmc.LOGERROR)
			...
			if results:
				for data in results:
					try:
						...
						# Magnet links require a hash attribute, otherwise Chains throws an exception an fails.
						...
						# Do not add the 'package' attribute, otherwise it estimates the file size per episode, and Orion already does that.
						# if data['file']['pack'] and type == Orion.TypeShow: item['package'] = 'season'

						if details: item['name_info'] = details
						else: item['name_info'] = ' ' # Not an empty string, since there is a bug.
