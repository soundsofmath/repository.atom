...
class Orion:
	...
	def streamVote(self, idItem, idStream, vote = VoteUp, automatic = False, notification = False):
		OrionItem.popularityVote(idItem = idItem, idStream = idStream, vote = vote, automatic = automatic, notification = notification)

	# Request the removal of a specific stream.
	def streamRemove(self, idItem, idStream, automatic = False, notification = False):
		OrionItem.remove(idItem = idItem, idStream = idStream, automatic = automatic, notification = notification)

	# Retrieve the supported stream types from the user settings.
	def streamTypes(self, supported = None):
		types = []
		app = self.mApp.id()
		setting = OrionSettings.getFiltersInteger('filters.stream.type', app if OrionSettings.getFiltersCustomEnabled(app) else None)
