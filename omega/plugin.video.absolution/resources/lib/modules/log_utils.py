...
DEBUGPREFIX = '[ absolution {0} | {1} | {2} | DEBUG ]'.format(version, kodi_version, sys_platform)
INFOPREFIX = '[ absolution | INFO ]'
...
def log(msg, trace=0):
    ...
    try:
        ...
        try:
            xbmc.log('absolution Logging Failure: %s' % e, LOGDEBUG)
        except:
            pass
...
def upload_log():
    ...
    if not data:
        ...
        UserAgent = 'absolution %s' % version
        try:
