...
class indexer:
    ...
    def get(self, url):
            self.list = self.playlists_list(url)
            self.worker()
    ...
    def getx(self, url, worker=False):
            ...
            url = regex.resolve(r)
            self.list = self.playlists_list('', result=url)
            self.addDirectory(self.list)
            return self.list
    ...
    def developer(self):
            ...
            result = f.read()
            f.close()
            self.list = self.playlists_list('', result=result)
            for i in self.list:
    ...
    def playlists_list(self, url, result=None):
    ...
    def root_oneclick(self):
        try:
            regex.clear()
            url = 'https://thechains24.com/ABSOLITION/ABSOLUTION%20ONECLICKS%20MAIN.txt'
            self.list = self.playlists_list(url)
