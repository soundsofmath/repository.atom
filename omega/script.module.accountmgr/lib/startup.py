        ...
        while True:
                ...
                if control.setting('api.service')=='true' and xbmcvfs.exists(var.chk_umb) and xbmcvfs.exists(var.chkset_umb) and str(var.chk_accountmgr_tk) != '':
                        try:
                                ...
                                chk_client = xbmcaddon.Addon('plugin.video.dynasty').getSetting("trakt.clientid")
                                ...
                                xbmc.log('%s: Dynasty API Failed!' % var.amgr, xbmc.LOGINFO)
                ...
                if control.setting('api.service')=='true' and xbmcvfs.exists(var.chk_thelabjr) and xbmcvfs.exists(var.chkset_thelabjr) and str(var.chk_accountmgr_tk) != '':
                        try:
                                with open(var.path_thelabjr) as f:
                                        if var.chk_api in f.read():
                                                pass
                                        else:
...
def restore_api():
        ...
        if xbmcvfs.exists(var.chk_thelabjr):
            try:
                with open(var.path_thelabjr,'r') as f:
                    data = f.read()
