...
class Auth:
    ...
    #Dynasty
                    try:
                        if 'Dynasty' in current:
                            if xbmcvfs.exists(var.chk_umb) and xbmcvfs.exists(var.chkset_umb):
                                    chk_auth_umb = xbmcaddon.Addon('plugin.video.dynasty').getSetting("trakt.user.token")
                                    if not str(var.chk_accountmgr_tk) == str(chk_auth_umb) or str(chk_auth_umb) == '':
                            ...
                            xbmc.log('%s: Dynasty Trakt Failed!' % var.amgr, xbmc.LOGINFO)
                            pass
    ...
    #TheLabJr
                try:
                        if 'TheLabJr' in current:
                            if xbmcvfs.exists(var.chk_thelabjr) and not xbmcvfs.exists(var.thelabjr_ud):
                                    os.mkdir(var.thelabjr_ud)
                                    xbmcvfs.copy(os.path.join(var.thelabjr), os.path.join(var.chkset_thelabjr))
                                    
                            if xbmcvfs.exists(var.chk_thelabjr) and not xbmcvfs.exists(var.chkset_thelabjr):
                                    xbmcvfs.copy(os.path.join(var.thelabjr), os.path.join(var.chkset_thelabjr))

                            if xbmcvfs.exists(var.chk_thelabjr) and xbmcvfs.exists(var.chkset_thelabjr):
                                    chk_auth_thelabjr = xbmcaddon.Addon('plugin.video.thelabjr').getSetting("trakt.token")
                                    if not str(var.chk_accountmgr_tk) == str(chk_auth_thelabjr) or str(chk_auth_thelabjr) == '':

                                            with open(var.path_thelabjr,'r') as f:
                                                data = f.read()

                                            client = data.replace(var.thelabjr_client,var.client_am).replace(var.thelabjr_secret,var.secret_am)

                                            with open(var.path_thelabjr,'w') as f:
                                                f.write(client)
