...
class sources:
    ...
    def getConstants(self):
        ...
        self.module_name = 'Shazam:'

        if externalEnabled == 'true':
            try:
                import shazamscrapers
                self.sourceDict += shazamscrapers.sources()
                self.module_name = 'shazamScrapers (' + str(control.addon('script.module.shazamscrapers').getSetting('package.folder')) + ' set):' \
                                   if control.addon('script.module.shazamscrapers').getSetting('package.folder') != 'shazamscrapers' else '[COLOR red]Shazam Is Searching[/COLOR]'
            except:
                pass
