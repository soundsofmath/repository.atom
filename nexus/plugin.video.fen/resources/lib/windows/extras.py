...
ratings_icons_type, enable_extra_ratings, extras_enabled_ratings = settings.extras_ratings_icons_type, settings.extras_enable_extra_ratings, settings.extras_enabled_ratings
...
class Extras(BaseDialog):
	...
	def make_ratings(self):
		...
		if not data: return
		def make_icon(_value): return 'common/flags/ratings/%s/%s.png' % (icon_type, _value)
		def set_properties(_prop, _id, _rating, _image):
			self.active_extra_ratings = True
			self.setProperty(_prop, 'true'), self.set_label(4000 + _id, _rating), self.set_image(4100 + _id, _image)
		icon_type, self.active_extra_ratings = ratings_icons_type(), False
		metascore_rating, tomatometer_rating, tomatousermeter_rating = data.get('metascore_rating', ''), data.get('tomatometer_rating', ''), data.get('tomatousermeter_rating', '')
		imdb_rating, tmdb_rating, tomato_image = data.get('imdb_rating', ''), self.rating, data.get('tomato_image', 'fresh')
		if metascore_rating and 'Meta' in current_settings: set_properties('metascore_rating', 1, '%s%%' % metascore_rating, make_icon('metacritic'))
		if tomatometer_rating and 'Tom/Critic' in current_settings:
			image = 'rtcertified' if tomato_image == 'certified' \
					else 'rtfresh' if (tomato_image == 'fresh' or int(tomatometer_rating) > 59) \
					else 'rtrotten'
			set_properties('tomatometer_rating', 2, '%s%%' % tomatometer_rating, make_icon(image))
		if tomatousermeter_rating and 'Tom/User' in current_settings:
			set_properties('tomatousermeter_rating', 3, '%s%%' % tomatousermeter_rating, make_icon('popcorn' if int(tomatousermeter_rating) > 59 else 'popcorn_spilt'))
		if imdb_rating and 'IMDb' in current_settings: set_properties('imdb_rating', 4, imdb_rating, make_icon('imdb'))
		if tmdb_rating and 'TMDb' in current_settings and self.active_extra_ratings: set_properties('tmdb_rating', 5, tmdb_rating, make_icon('tmdb'))
		if self.active_extra_ratings:
	...
	def set_starting_constants(self, kwargs):
		...
		self.active_extra_ratings, self.selected = False, None
